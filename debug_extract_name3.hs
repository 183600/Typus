...
-- Test the original extractName logic
...
main = do
    ...
    let simpleCode = "println(s1)"
    let simpleParenIdx = length (takeWhile (/= '(') simpleCode)
    let simpleName = extractName simpleCode simpleParenIdx (simpleParenIdx - 1)
    putStrLn $ "Simple extracted name: " ++ show simpleName