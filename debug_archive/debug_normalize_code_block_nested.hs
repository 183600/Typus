...
-- 测试normalizeIndentation code block
...
test_normalize_indentation_code_block = do
  ...
  putStrLn $ "All non-comment lines not starting with 4 spaces: " ++ show (all (not . isPrefixOf "    ") nonCommentLines)
  putStrLn $ "Normalized not null: " ++ show (not (null normalized))

-- 测试normalizeIndentation nested
...
test_normalize_indentation_nested = do
  ...
  putStrLn $ "Normalized: " ++ show normalized
  putStrLn $ "NormLines: " ++ show normLines
  putStrLn $ "All lines not starting with 4 spaces: " ++ show (all (not . isPrefixOf "    ") normLines)
  putStrLn $ "Normalized not null: " ++ show (not (null normalized))
