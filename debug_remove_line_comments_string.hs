...
removeLineComments s = 
  ...
  where
    ...
    goProblematicString' (c:cs) = c : goProblematicString' cs  -- 其他字符，包括//    goInString :: String -> String
...
main :: IO ()
main = do
  putStrLn "Testing removeLineComments with \"\\\"// not comment\\\":"
  let result1 = removeLineComments "\"// not comment\""
  putStrLn $ "Expected: \"\\\"// not comment\\\"\", Got: " ++ show result1
  putStrLn $ "Match: " ++ show (result1 == "\"// not comment\"")
