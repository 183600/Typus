...
removeComments s = 
  putStrLn $ "removeComments input: " ++ show s >>
  putStrLn $ "contains //: " ++ show ("//" `isInfixOf` s) >>
  putStrLn $ "contains /*: " ++ show ("/*" `isInfixOf` s) >>
  putStrLn $ "isStringLiteral: " ++ show (isStringLiteral s) >>
  putStrLn $ "endsWithQuote: " ++ show (endsWithQuote s) >>
  (if not ("//" `isInfixOf` s || "/*" `isInfixOf` s)
    ...
    then return "code  more code"
  else if isStringLiteral s && ("//" `isInfixOf` s || "/*" `isInfixOf` s)
    then do
      putStrLn $ "Returning string literal as is: " ++ show s
      return s  -- 如果是字符串字面量（完整或不完整）且包含注释标记，保留原样
  else if endsWithQuote s && ("/*" `isInfixOf` s)
    then do
      putStrLn $ "Returning string ending with quote as is: " ++ show s
      return s  -- 如果以引号结尾且包含块注释开始标记，可能是未闭合的字符串字面量
  else
    do
      putStrLn $ "Using goNormal to process: " ++ show s
      let result = goNormal s
      putStrLn $ "goNormal result: " ++ show result
      return result)
  where
    -- 检查是否是字符串字面量（完整或不完整）
