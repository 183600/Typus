...
removeLineComments s = 
  putStrLn $ "removeLineComments input: " ++ show s >>
  if null s  -- 空字符串
    then s  -- 保持空字符串不变
  else if s == "\n"  -- 特殊情况：只有换行符
    then s  -- 保持换行符不变
  ...
  where
    ...
    preserveLineCount input = 
      putStrLn $ "preserveLineCount input: " ++ show input >>
    ...
    processLine line = 
      putStrLn $ "processLine input: " ++ show line >>
      if null line
        then line  -- 空行保持不变
    ...
    removeSingleLineComments ('"':xs) = 
      putStrLn $ "Found quote, remaining: " ++ show xs >>
      putStrLn $ "isProblematicUnclosedString: " ++ show (isProblematicUnclosedString ('"':xs)) >>
      -- 检查是否是问题性的未闭合字符串
    ...
    removeSingleLineComments ('/':'/':xs) = 
      putStrLn $ "Found comment, returning empty" >>
      case xs of
        [] -> []  -- 只有"//"的情况
        _ -> []  -- 有注释内容的情况
    ...
    goProblematicString [] = []
    goProblematicString ('\n':cs) = 
      putStrLn $ "goProblematicString found newline, remaining: " ++ show cs >>
...
-- 处理问题性字符串换行后的内容，不把//当作注释
    goProblematicString' :: String -> String
    goProblematicString' [] = []
    goProblematicString' ('"':xs) = 
      putStrLn $ "goProblematicString' found closing quote, remaining: " ++ show xs >>
      '"' : removeSingleLineComments xs  -- 遇到字符串结束，返回正常处理
