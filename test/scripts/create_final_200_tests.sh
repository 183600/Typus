...
# 创建新文件
cat > test/Test/Unit/Final200QuickCheckTests.hs << 'EOF'
...
for i in {1..40}; do
  if [ $counter -eq 1 ]; then
    echo "  [ testProperty \"core utils test $i\" prop_trim_idempotent" >> test/Test/Unit/Final200QuickCheckTests.hs
  else
    echo "  , testProperty \"core utils test $i\" prop_trim_idempotent" >> test/Test/Unit/Final200QuickCheckTests.hs
  fi
  counter=$((counter + 1))
...
for i in {1..40}; do
  echo "  , testProperty \"parser test $i\" prop_parse_identifier_basic" >> test/Test/Unit/Final200QuickCheckTests.hs
  counter=$((counter + 1))
done

for i in {1..40}; do
  echo "  , testProperty \"compiler core test $i\" prop_compile_basic" >> test/Test/Unit/Final200QuickCheckTests.hs
  counter=$((counter + 1))
done

for i in {1..40}; do
  echo "  , testProperty \"dependency analysis test $i\" prop_dependency_analysis_basic" >> test/Test/Unit/Final200QuickCheckTests.hs
  counter=$((counter + 1))
done

for i in {1..40}; do
  echo "  , testProperty \"ownership analysis test $i\" prop_ownership_analysis_basic" >> test/Test/Unit/Final200QuickCheckTests.hs
  counter=$((counter + 1))
done

for i in {1..40}; do
  if [ $i -eq 30 ]; then
    echo "  , testProperty \"error handling test $i\" prop_error_handler_basic" >> test/Test/Unit/Final200QuickCheckTests.hs
  else
    echo "  , testProperty \"error handling test $i\" prop_error_handler_basic" >> test/Test/Unit/Final200QuickCheckTests.hs
  fi
done
...
# 关闭列表
echo "  ]" >> test/Test/Unit/Final200QuickCheckTests.hs
