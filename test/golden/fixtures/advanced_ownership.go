...
type LifetimeObject[T any] struct {
    data *T
    created time.Time
...
type ThreadSafeOwned[T any] struct {
    data *T
    mutex sync.Mutex
    owner int
    moved bool
...
type ChannelOwned[T any] struct {
    data chan T
    sent bool
}
...
type OwnedWithLifetime[T any] Lifetime> where Lifetime > time.Duration(0) struct {
    data *T
...
type ReferenceCounted[T any] struct {
    data *T
