...
newBoundaryConditionSpec = testGroup "New Boundary Condition Tests"
  [ testCase "Empty and whitespace-only inputs" $ do
      -- Empty string
      case parseTypus "" of
        Left _ -> return ()  -- Expected to fail
        Right typusFile -> 
          assertBool "Empty input should result in empty blocks" $ null (tfBlocks typusFile)
      
      -- Whitespace only
      case parseTypus "   \n\t  \n  " of
        Left _ -> return ()  -- Expected to fail
        Right typusFile -> 
          assertBool "Whitespace-only should result in empty blocks" $ null (tfBlocks typusFile)
  ...
  , testCase "Extremely long lines and inputs" $ do
      ...
      case parseTypus codeWithManyLines of
        ...
        Right typusFile -> 
          assertBool "Should handle many lines" $ not (null (tfBlocks typusFile))
  
  , testCase "Nested and malformed directives" $ do
      ...
      case parseTypus nestedDirectives of
        ...
        Right typusFile -> do
          ...
          assertBool "Should have nested blocks" $ length blocks >= 1
  ...
  , testCase "Unicode and special characters" $ do
      ...
      case parseTypus unicodeCode of
        ...
        Right typusFile -> 
          assertBool "Should handle Unicode characters" $ not (null (tfBlocks typusFile))
      ...
      case parseTypus specialChars of
        ...
        Right typusFile -> 
          assertBool "Should handle special characters" $ not (null (tfBlocks typusFile))
...
prop_split_edge_cases delim s = 
  let parts = splitBy delim s
      ...
      noEmptyCollapsed = all (not . null) collapsed
      -- Original should have length equal to count of delimiters + 1
      expectedLength = length (filter (== delim) s) + 1
  in noEmptyCollapsed && length parts == expectedLength
...
prop_error_boundary_positions (Positive line) (Positive col) = 
  line <= 1000 && col <= 1000 ==> 
    ...
    in expectedLocation `isInfixOf` formatted
