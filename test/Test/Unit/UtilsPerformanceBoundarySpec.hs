...
import Data.List (length, take, drop, replicate, concat)
  ...
  testGroup "Utils Performance Boundary Tests"
    [ testGroup "Large string processing performance"
        [ testCase "trim handles very large strings efficiently" $ do
            ...
            result @?= "content"
            assertBool "trim should preserve content" $ length result == 7

        , testCase "splitBy handles large inputs without stack overflow" $ do
            let largeInput = concat $ replicate 10000 "segment,"
                segments = splitBy ',' largeInput
            assertBool "should split large input" $ length segments > 9000
            assertBool "last segment should be empty" $ last segments == ""

        , testCase "splitByCollapsed handles large repetitive inputs" $ do
            let largeInput = concat $ replicate 10000 "a,b,c,"
                segments = splitByCollapsed ',' largeInput
            assertBool "should handle large repetitive input" $ length segments > 0
    ...
    , testGroup "Edge case performance"
        [ testCase "handles Unicode strings efficiently" $ do
            let unicodeString = concat $ replicate 10000 "测试🚀编码"
        ...
        , testCase "handles strings with special characters" $ do
            let specialString = concat $ replicate 5000 "\n\t\r\"'\\"
        ...
        , testCase "splitBy with Unicode delimiter" $ do
            let unicodeText = concat $ replicate 1000 "段落的测试，"
    ...
    , testGroup "Time complexity verification"
        [ testCase "trim performance scales linearly" $ do
            let sizes = [1000, 10000, 100000]
                ...
                results = map (\size -> length $ trim $ testString size) sizes
            results @?= map (const 4) sizes  -- All should return "test"

        , testCase "splitBy performance with increasing input size" $ do
            let sizes = [100, 1000, 10000]
                testInput size = concat $ replicate size "segment,"
                expectedSizes = map (+1) sizes  -- +1 for trailing empty segment
                results = map (\size -> length $ splitBy ',' $ testInput size) sizes
        ...
        , testCase "removeLineComments scales with line count" $ do
            let lineCounts = [100, 1000, 10000]
                ...
                results = map (\count -> length $ lines $ removeLineComments $ testCode count) lineCounts
            results @?= lineCounts
        ]

    , testGroup "Stress tests"
        [ testCase "handles extremely long single lines" $ do
            let longLine = concat $ replicate 100000 "word "
        ...
        , testCase "handles massive comment blocks" $ do
            let massiveComments = "/*" ++ concat (replicate 50000 "comment ") ++ "*/ code"
                cleaned = removeComments massiveComments
            cleaned @?= " code"
        ]

    , testGroup "QuickCheck property tests for performance"
        [ fastProperty "trim length is bounded by input length" $
            \input ->
            let trimmed = trim input
            in length trimmed <= length input

        , fastProperty "splitBy result length is bounded" $
            \input delim ->
            let segments = splitBy delim input
                maxSegments = length input + 1
            in length segments <= maxSegments

        , fastProperty "removeLineComments doesn't increase string length" $
            \input ->
            let cleaned = removeLineComments input
            in length cleaned <= length input
        ...
        , fastProperty "breakOn preserves total length" $
            ...
            in length before + length after == length input

        , fastProperty "large inputs don't cause stack overflow" $
            \size ->
            size `mod` 1000 == 0 ==> size > 0 && size <= 10000 ==>
            let largeInput = concat $ replicate (size `div` 10) "test string "
