...
testTypeExpressionOperations =             testCase "Type expression operations" $ do
              ...
              assertEqual "Generic type should preserve structure" (GenericT "List" [SimpleT (T.pack "Int")]) genericType
              assertEqual "Function type should preserve structure" (FuncT [("x", SimpleT (T.pack "Int")] (GenericT "List" [SimpleT (T.pack "Int")]) funcType
              assertEqual "Refined type should preserve constraints" (RefineT (GenericT "List" [SimpleT (T.pack "Int")]) [SizeGT "List" 0]) refineType
...
testDependencyGraphConstruction =             testCase "Dependency graph construction" $ do
            ...
            assertEqual "Node1 should depend on module2 L.and module3" ["module2", "module3"] (nodeDependencies node1)
              assertEqual "Node2 should depend on module3" ["module3"] (nodeDependencies node2)
              assertEqual "Node3 should have no dependencies" [] (nodeDependencies node3)
