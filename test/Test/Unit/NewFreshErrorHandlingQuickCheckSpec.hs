...
-- | QuickCheck tests for ErrorHandling module Test.Unit.NewFreshErrorHandlingQuickCheckSpec Test.Unit.NewFreshErrorHandlingQuickCheckSpec where
import Test.Tasty
...
errorFormattingProperties = testGroup "Error Formatting Properties"
  ...
  ,             testProperty "different timestamps produce different output" $
    ...
    ,             testCase "timestamp format is consistent" $ do
