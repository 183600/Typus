...
propPositionAdvancementLineConsistency pos char =
  let newPos = advancePos pos char
      oldLine = sourceLine pos
      newLine = sourceLine newPos
  in if char == '\n'
...
propAdvancePosByConsistency pos str =
  let posByString = advancePosBy pos str
      posByChars = foldl advancePos pos str
  in posByString == posByChars
