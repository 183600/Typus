...
module Test.Unit.EnhancedOwnershipBoundaryQuickCheckSpec (tests) where
...
-- ============================================================================
-- Ownership Boundary QuickCheck Tests
-- ============================================================================

tests :: TestTree
tests = testGroup "Ownership Boundary QuickCheck Tests"
  [ testProperty "analyzeOwnership handles empty input" prop_analyze_empty_input
  , testProperty "analyzeOwnership handles whitespace-only input" prop_analyze_whitespace_input
  , testProperty "analyzeOwnership detects use after move" prop_detects_use_after_move
  , testProperty "analyzeOwnership detects double move" prop_detects_double_move
  , testProperty "analyzeOwnership handles borrow conflicts" prop_handles_borrow_conflicts
  , testProperty "analyzeOwnership respects variable scope" prop_respects_variable_scope
  , testProperty "lexAll handles malformed input gracefully" prop_lex_malformed_input
  , testProperty "parseProgram handles syntax errors gracefully" prop_parse_syntax_errors
  , testProperty "ownership transfers are tracked correctly" prop_ownership_transfers_tracked
  , testProperty "built-in functions don't cause ownership errors" prop_builtin_functions_safe
  , testProperty "complex ownership scenarios are handled" prop_complex_scenarios_handled
  , testCase "ownership analysis handles edge cases" test_ownership_edge_cases
  , testCase "ownership error formatting works" test_error_formatting
  ]
