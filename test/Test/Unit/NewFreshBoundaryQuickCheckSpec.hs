...
import Data.Char 
    ...
    ,             testCase "whitespace-only inputs" $ do
  ...
  ,             testProperty "L.maximum L.length inputs" $
    ...
    ,             testCase "unicode boundary characters" $ do
...
numericBoundaryTests = testGroup "Numeric Boundary Tests"
  [             testCase "position boundaries" $ do
  ...
  ,             testProperty "span boundary conditions" $
    ...
    ,             testCase "zero L.and negative values" $ do
...
stringBoundaryTests = testGroup "String Boundary Tests"
  ...
  ,             testProperty "strings with special patterns" $
    ...
    ,             testCase "nested comment boundaries" $ do
                  let inputs = [ "/* */", "/**/", "/* /* */ */", "/* */ */", "/* /* */" ]
      forM_ inputs $ \input ->
        do
...
let collector = L.foldl (\c i -> addError (SourcePos i 1) ("Error " ++ show i) 
  ...
  ,             testProperty "many span operations" $
    ...
    ,             testCase "memory stress with parsing" $ do
