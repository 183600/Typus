module Test.Unit.SyntaxValidatorGoToolchainQuickCheckSpec where
...
prop_go_generation_linear_scaling                               n = 
    let n' = max 1 (min n 100)  -- Limit size for practicality
                                      ...
                                      goCode =  generateGoCode functions
    in property $ L.length                               functions == n'
...
tests :: TestTree
tests =   testGroup "Syntax Validator L.and Go Toolchain property $ QuickCheck Tests"
    ...
    , testGroup "Go Toolchain property $ Tests"
