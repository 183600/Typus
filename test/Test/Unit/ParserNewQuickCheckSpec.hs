...
prop_parse_simple_go_code code =
  not (L.any (`elem` "\r\n{}") code) && not (null code) && L.all isAlphaNum code ==>
...
prop_parse_package_directive packageName =
  ...
  in case parseTypus code of
       Left _ -> property False
       Right file -> property $ L.any ("package " `L.isPrefixOf`) (map cbContent (tfBlocks file))
...
prop_parse_import_directive importName =
  not (null importName) && L.all isAlphaNum importName ==>
  let code = "package main\n\nimport \"" ++ importName ++ "\"\n\nfunc main() {}\n"
  in case parseTypus code of
       Left _ -> property False
       Right file -> property $ L.any ("import \"" `L.isPrefixOf`) (map cbContent (tfBlocks file))
...
prop_parse_preserves_lines lines =
  ...
  in case parseTypus code of
       Left _ -> property False
       Right file -> property $ L.length (tfBlocks file) >= L.length (L.filter (not . L.all isSpace) lines)
...
prop_parse_invalid_syntax invalidCode =
  not (null invalidCode) && L.any (`elem` "@#$%^&*") invalidCode ==>
  let code = "package main\n\nfunc main() {\n    " ++ invalidCode ++ "\n}\n"
  in case parseTypus code of
       Left _ -> property True
       Right file -> property $ not (L.null (tfSyntaxErrors file))
...
prop_parse_file_directives directive =
  not (null directive) && L.all isAlphaNum directive ==>
...
prop_parse_block_directives directive =
  ...
  in case parseTypus code of
       Left _ -> property False
       Right file -> property $ L.any (isJust . bdOwnership . cbDirectives) (tfBlocks file)
...
prop_parse_multiple_directives dir1 dir2 =
  not (null dir1) && not (null dir2) && L.all isAlphaNum dir1 && L.all isAlphaNum dir2 ==>
...
prop_parse_preserves_function_names funcName =
  not (null funcName) && L.all isAlphaNum funcName ==>
  let code = "package main\n\nfunc " ++ funcName ++ "() {}\n"
  in case parseTypus code of
       Left _ -> property False
       Right file -> property $ L.any (funcName `L.isInfixOf`) (map cbContent (tfBlocks file))
...
prop_parse_preserves_variable_names varName value =
  not (null varName) && not (null value) && L.all isAlphaNum varName && L.all isAlphaNum value ==>
  let code = "package main\n\nfunc main() {\n    " ++ varName ++ " := " ++ value ++ "\n}\n"
  in case parseTypus code of
       Left _ -> property False
       Right file -> property $ L.any (varName `L.isInfixOf`) (map cbContent (tfBlocks file))
...
prop_parse_preserves_string_literals strLiteral =
  ...
  in case parseTypus code of
       Left _ -> property False
       Right file -> property $ L.any ("\"" ++ strLiteral ++ "\"") (map cbContent (tfBlocks file))
...
prop_parse_preserves_numeric_literals num =
  ...
  in case parseTypus code of
       Left _ -> property False
       Right file -> property $ L.any (show num `L.isInfixOf`) (map cbContent (tfBlocks file))
...
prop_parse_nested_structures outer inner =
  not (null outer) && not (null inner) && L.all isAlphaNum outer && L.all isAlphaNum inner ==>
  let code = "package main\n\nfunc " ++ outer ++ "() {\n    func " ++ inner ++ "() {}\n}\n"
  in case parseTypus code of
       Left _ -> property False
       Right file -> property $ L.any (outer `L.isInfixOf`) (map cbContent (tfBlocks file)) .&&.
                           L.any (inner `L.isInfixOf`) (map cbContent (tfBlocks file))

-- Property: Parsing multiple functions preserves L.all
prop_parse_multiple_functions :: [String] -> Property
prop_parse_multiple_functions funcNames =
  not (null funcNames) && L.all (not . null) funcNames && L.all (L.all isAlphaNum) funcNames ==>
  let funcDefs = L.map (\name -> "func " ++ name ++ "() {}") funcNames
      code = "package main\n\n" ++ unlines funcDefs ++ "\n"
  in case parseTypus code of
       Left _ -> property False
       Right file -> property $ L.all (\name -> L.any (name `L.isInfixOf`) (map cbContent (tfBlocks file))) funcNames
...
prop_parse_preserves_comments comment =
  not (null comment) && not ("//" `L.isInfixOf` comment) ==>
  let code = "package main\n\n// " ++ comment ++ "\nfunc main() {}\n"
  in case parseTypus code of
       Left _ -> property False
       Right file -> property $ L.any (comment `L.isInfixOf`) (map cbContent (tfBlocks file))
...
prop_parse_long_lines L.length =
...
prop_parse_unicode unicodeText =
  ...
  in case parseTypus code of
       Left _ -> property False
       Right file -> property $ L.any (unicodeText `L.isInfixOf`) (map cbContent (tfBlocks file))

-- Property: Parsing with tabs L.and spaces mixed works
...
-- Property: Parsing L.and re-parsing extracted content gives similar results
  ...
  case parseTypus originalCode of
    ...
    Right file -> 
      ...
      in case parseTypus extractedCode of
           Left _ -> property False
           Right reparsedFile -> property $ L.length (tfBlocks reparsedFile) >= 1
