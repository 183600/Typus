...
import SourceLocation (SourcePos(..), SourceSpan(..), Located(..), 
                       spanStart, spanEnd, locatedWithSpan, posAt, 
...
prop_span_start_end_consistency startLine startCol endLine endCol =
  let start = posAt startLine startCol
      end = posAt endLine endCol
      span = SourceSpan start end
  in property $ 
    (spanStart span == start) && (spanEnd span == end)

prop_span_ordering :: Int -> Int -> Int -> Int -> Int -> Int -> Int -> Property
prop_span_ordering sLine1 sCol1 eLine1 eCol1 sLine2 sCol2 eCol2 =
  let start1 = posAt sLine1 sCol1
      end1 = posAt eLine1 eCol1
      start2 = posAt sLine2 sCol2
      end2 = posAt (sLine2 + eCol2) (sCol2 + eCol2)  -- Ensure end2 >= start2
      span1 = SourceSpan start1 end1
      span2 = SourceSpan start2 end2
      spanOrdering = span1 `compare` span2
      startPosOrdering = start1 `compare` start2
  in property $ spanOrdering == startPosOrdering
...
prop_located_value_extraction line1 col1 line2 col2 value =
  let start = posAt line1 col1
      ...
      located = locatedWithSpan (SourceSpan start end) value
...
prop_span_contains_itself sLine sCol eLine eCol =
  let start = posAt sLine sCol
      end = posAt (max sLine eLine) (max sCol eCol)  -- Ensure end >= start
      span = SourceSpan start end
...
prop_located_preserves_value sLine sCol eLine eCol value =
  let start = posAt sLine sCol
      end = posAt (max sLine eLine) (max sCol eCol)
      located = locatedWithSpan (SourceSpan start end) value
  in property $ locatedValue located == value

-- | Test span ordering transitivity
prop_span_ordering_transitive :: Int -> Int -> Int -> Int -> Property
prop_span_ordering_transitive sLine1 sCol1 sLine2 sCol2 =
  let start1 = posAt sLine1 sCol1
      end1 = posAt (sLine1 + 1) sCol1
      start2 = posAt sLine2 sCol2
      end2 = posAt (sLine2 + 1) sCol2
      start3 = posAt (max sLine1 sLine2) (max sCol1 sCol2)
      end3 = posAt (max sLine1 sLine2 + 1) (max sCol1 sCol2)
      span1 = SourceSpan start1 end1
      span2 = SourceSpan start2 end2
      span3 = SourceSpan start3 end3
  in property $ (span1 <= span3 && span2 <= span3) ==> (span1 <= span3 || span2 <= span3)
