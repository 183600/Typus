module Test.Unit.NewCabalTestCasesSpec where


import Test.Tasty 
import Test.Tasty.HUnit (testCase, assertFailure, assertBool, (@?=)), assertFailure, assertString
...
in property $ property $ not (null name) && size >= 0