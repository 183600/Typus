...
prop_collect_symbols_basic code =
  not (null code) ==>
  let result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes code) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (property (property (Map.size table >= 0)))

-- | 测试符号收集的幂等性
prop_collect_symbols_idempotent :: String -> Property
prop_collect_symbols_idempotent code =
  not (null code) ==>
  let result1 = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes code) newAnalyzerState))
      result2 = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes code) newAnalyzerState))
  in case (result1, result2) of
    (Right table1, Right table2) -> property (Map.size table1 === Map.size table2)
...
prop_collect_symbols_empty =
  let result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes "") newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (Map.size table === 0)
...
prop_collect_symbols_single_line code =
  ...
  in case result of
    Left _ -> property True
    Right table -> property (property (Map.size table >= 0))
...
prop_collect_symbols_multiline (Positive n) code =
  ...
  let multiLineCode = unlines $ replicate n code
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes multiLineCode) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (property (Map.size table >= 0))
...
prop_collect_symbols_type_def typeName =
  ...
  let typeDef = "type " ++ typeName ++ " = int"
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes typeDef) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (property (Map.size table >= 0))
...
prop_collect_symbols_var_decl varName =
  ...
  let varDecl = "var " ++ varName ++ " : int"
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes varDecl) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (property (Map.size table >= 0))
...
prop_collect_symbols_func_decl funcName =
  ...
  let funcDecl = "func " ++ funcName ++ "(x: int) : int"
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes funcDecl) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (property (Map.size table >= 0))
...
prop_collect_symbols_complex_type =
  let complexType = "type Complex = func(int, ref(string)) -> array[int, 10]"
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes complexType) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (property (Map.size table >= 0))
...
prop_collect_symbols_recursive_type typeName =
  ...
  let recursiveType = "type " ++ typeName ++ " = " ++ typeName ++ " | nil"
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes recursiveType) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (property (Map.size table >= 0))
...
prop_collect_symbols_generic_type typeName =
  ...
  let genericType = "type " ++ typeName ++ "[T] = T | " ++ typeName ++ "[T]"
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes genericType) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (property (Map.size table >= 0))
...
prop_collect_symbols_complex_program =
  let complexProgram = unlines
      ...
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes complexProgram) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (property (Map.size table >= 0))
...
prop_collect_symbols_invalid_code invalidCode =
  let result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes invalidCode) newAnalyzerState))
  in case result of
...
prop_collect_symbols_duplicate_defs name =
  ...
  let duplicateDefs = "type " ++ name ++ " = int\ntype " ++ name ++ " = string"
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes duplicateDefs) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (property (Map.size table >= 0))
...
prop_collect_symbols_mutual_deps name1 name2 =
  ...
  let mutualDeps = "type " ++ name1 ++ " = " ++ name2 ++ "\ntype " ++ name2 ++ " = " ++ name1
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes mutualDeps) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (Map.size table >= 2)
...
prop_collect_symbols_nested_types typeName =
  ...
  let nestedType = "type " ++ typeName ++ " = {\n  field: int\n  nested: " ++ typeName ++ "\n}"
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes nestedType) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (property (Map.size table >= 0))
...
prop_collect_symbols_many_symbols (Positive n) =
  ...
  let manySymbols = unlines ["type Type" ++ show i ++ " = int" | i <- [1..n]]
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes manySymbols) newAnalyzerState))
  in case result of
    Left _ -> property True
    Right table -> property (Map.size table >= n)
...
test_collect_symbols_empty = do
  ...
  case result of
    ...
    Right table -> assertEqual "Empty code should result in empty symbol table" 0 (Map.size table)
...
test_collect_symbols_simple_type = do
  ...
  case result of
    ...
    Right table -> assertBool "Simple type definition should result in non-empty symbol table" (Map.size table > 0)
...
test_collect_symbols_invalid_code = do
  let invalidCode = "this is not valid code"
      result = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes invalidCode) newAnalyzerState))
  case result of
    Left _ -> assertBool "Invalid code should result in error" True
    Right table -> assertBool "Invalid code might still result in symbol table" (Map.size table >= 0)
...
test_collect_symbols_consistency = do
  let code = "type MyInt = int"
      result1 = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes code) newAnalyzerState))
      result2 = unsafePerformIO (runExceptT (evalStateT (collectSymbolsAndTypes code) newAnalyzerState))
  case (result1, result2) of
    (Right table1, Right table2) -> assertEqual "Collection should be consistent" (Map.size table1) (Map.size table2)
