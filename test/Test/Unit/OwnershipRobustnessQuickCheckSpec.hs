...
-- | Test ownership transfer invariants
testOwnershipTransferInvariants :: Property
testOwnershipTransferInvariants =
  forAll arbitrary $ \initialState ->
    forAll arbitrary $ \transfer ->
      let newState = Own.performTransfer initialSate transfer
          transferredResources = Own.getTransferredResources transfer
