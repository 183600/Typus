...
prop_trim_no_leading_trailing_spaces s = 
  let trimmed = trim s
  in null trimmed || 
     (not (null (head trimmed)) && head trimmed `notElem` " \t\n\r") &&
     (not (null (last trimmed)) && last trimmed `notElem` " \t\n\r")

-- 测试splitBy函数的性质
prop_split_by_empty_delim :: Char -> String -> Bool
prop_split_by_empty_delim delim s = length (splitBy delim s) >= 1
...
prop_remove_line_comments_preserves_non_comment s = 
  ...
  in length linesBefore >= length linesAfter
...
tests = testGroup "Utils QuickCheck Tests"
  ...
  , testProperty "trim and split consistency" prop_trim_split_consistency
  ]