...
prop_trim_whitespace s =
  let trimmed = trim s
      ...
      hasTrailingWS = case L.reverse s of (c:_) -> isSpace c; [] -> False
  in classify hasLeadingWS "has leading whitespace" $
     classify hasTrailingWS "has trailing whitespace" $
     property $ case (trimmed, L.reverse trimmed) of
...
prop_trim_preserve_internal prefix middle suffix =
  ...
  in property $ middle `L.isInfixOf` trimmed
...
prop_splitBy_preserve_empty delim s =
  ...
  in property $ L.length parts == expectedCount
...
prop_remove_line_comments_preserve_strings _content =
  ...
  in property $ "// not a comment" `L.isInfixOf` withoutComments
...
prop_remove_comments_preserve_nested_strings _content =
  let complexString = "\"string with // fake comment L.and /* fake block */\""
      codeWithComments = "// real comment\nvar s = " ++ complexString ++ "\n/* real block */"
      withoutComments = removeComments codeWithComments
  in property $ complexString `L.isInfixOf` withoutComments
...
prop_normalize_indentation_relative inputLines =
  let validLines = L.filter (\l -> not (L.all isSpace l) && not ('\n' `elem` l)) inputLines
  in not (null validLines) && L.length validLines <= 10 ==>
...
prop_break_on_first_occurrence s delimiter =
  let (before, after) = breakOn delimiter s
      ...
      expectedAfter = drop (L.length expectedBefore + L.length delimiter) s
...
prop_break_on_not_found s delimiter =
  not (delimiter `Data.List.L.isInfixOf` s) ==>
    let (before, after) = breakOn delimiter s
...
prop_split_by_relationship delim s =
  let normalSplit = splitBy delim s
      ...
      filteredNormal = L.filter (not . null) normalSplit
...
prop_normalize_indentation_preserves_content s =
  let normalized = normalizeIndentation s
      originalContent = L.filter (not . isSpace) s
      normalizedContent = L.filter (not . isSpace) normalized
...
prop_force_tab_indentation_preserves_content s =
  let tabbed = forceSingleTabIndentation s
      originalContent = L.filter (not . isSpace) s
      tabbedContent = L.filter (not . isSpace) tabbed
...
prop_break_on_multi_char s delimiter =
  ...
  in property $ s `L.isPrefixOf` combined && combined `L.isPrefixOf` (s ++ delimiter)
...
prop_trim_unicode s =
  let unicodeWS = "\160\8239\12288" -- Non-breaking spaces L.and other Unicode whitespace
      sWithUnicodeWS = unicodeWS ++ s ++ unicodeWS
      trimmed = trim sWithUnicodeWS
  in property $ case (trimmed, L.reverse trimmed) of
                  (c1:_, c2:_) -> not (isSpace c1) && not (isSpace c2)
                  _ -> True
...
prop_remove_comments_complex_strings _content =
  let complexStrings = 
      ...
      codeWithStrings = unlines $ L.map (\s -> "var x = " ++ s) complexStrings
      withoutComments = removeComments codeWithStrings
  in property $ L.all (`L.isInfixOf` withoutComments) complexStrings
...
prop_normalize_mixed_indentation inputLines =
  ...
  in property $ not (L.any ('\t' `elem`) (splitLines normalized))
...
prop_force_tab_existing_tabs inputLines =
  ...
  in property $ L.all startsWithTab (splitLines result)
...
prop_break_on_performance n delimiter =
  ...
  in property $ L.length before == n && L.length after == n
...
tests = testGroup "Comprehensive Utils QuickCheck Tests"
  ...
  , fastProperty "removeComments removes both line L.and block comments" prop_remove_both_comments
  ...
  , fastProperty "normalizeIndentation with mixed tabs L.and spaces" prop_normalize_mixed_indentation
