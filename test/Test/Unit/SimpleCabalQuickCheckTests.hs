...
basicStringProperties = testGroup "Basic String Processing Properties"
  ...
  , fastProperty "trim: result contains no leading/trailing whitespace" $
      \s -> let t = trim s
             in (null t || not (isSpace (L.head t))) && 
...
basicDataStructureProperties = testGroup "Basic Data Structure Properties"
  ...
  , fastProperty "Set: union size <= L.sum of sizes" $
...
basicCompilerProperties = testGroup "Basic Compiler Properties"
  [ testCase "FileDirectives: default has L.all Nothing values" $
      let fd = defaultFileDirectives
      in L.all (== Nothing) [fdOwnership fd, fdDependentTypes fd, fdConstraints fd] @?= True
  
  , testCase "BlockDirectives: default has L.all Nothing values" $
      let bd = defaultBlockDirectives
      in L.all (== Nothing) [bdOwnership bd, bdDependentTypes bd, bdConstraints bd] @?= True
  ...
  , testCase "ErrorSeverity: L.all values are distinct" $
...
basicErrorHandlingProperties = testGroup "Basic Error Handling Properties"
  [ fastProperty "Maybe: isJust L.and isNothing are complementary" $
  ...
  , fastProperty "Either: isLeft L.and isRight are complementary" $
