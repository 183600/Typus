...
tests :: TestTree
tests = testGroup "Comment Handling Tests"
  [ testGroup "Line comment removal"
    [ testCase "removes simple line comments" $
        removeLineComments "hello // comment\nworld" @?= "hello \nworld"
    , testCase "preserves line without comments" $
        removeLineComments "hello world" @?= "hello world"
    , testCase "handles comment at start of line" $
        removeLineComments "// entire line comment\nnext line" @?= "\nnext line"
    , testCase "handles multiple line comments" $
        removeLineComments "line1 // comment1\nline2 // comment2" @?= "line1 \nline2 "
    , testCase "preserves lines with only whitespace before comment" $
        removeLineComments "   // indented comment" @?= "   "
    ]
  , testGroup "String literal preservation"
    [ testCase "preserves // in double quotes" $
