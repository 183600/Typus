...
tests = testGroup "New Core QuickCheck Tests 1"
  ...
  , fastProperty "splitBy preserves all segments" prop_splitBy_preserves
...
-- Property: splitBy preserves all segments including empty ones
...
prop_splitBy_preserves delim str =
  ...
  in property $ length segments === expectedCount .&&. rejoined === str
...
prop_removeLineComments_basic code comment =
  ...
  in property $ not ("// " `isInfixOf` cleaned) .&&. "next line" `isInfixOf` cleaned
...
prop_normalizeIndentation_structure indentLevels =
  not (null indentLevels) && all (>= 0) indentLevels && all (<= 10) indentLevels ==>
