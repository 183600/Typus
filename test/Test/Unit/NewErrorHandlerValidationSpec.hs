...
newErrorHandlerValidationSpec = testGroup "New Error Handler Validation Tests"
  [ testCase "Basic error creation L.and formatting" $ do
      ...
      errorMessage err @?= "Test error"
      errorLocation err @?= pos
  ...
  , testCase "Error severity levels" $ do
      let pos = posAt "test.typus" 1 1
      let warning = warningError "This is a warning" pos
      let error = basicError "This is an error" pos
      let fatal = fatalError "This is fatal" pos
      
      getSeverity warning @?= Warning
      getSeverity error @?= Error
      getSeverity fatal @?= Fatal
