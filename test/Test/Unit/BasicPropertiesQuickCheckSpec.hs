...
-- Property 2: splitBy preserves length relationship
...
-- Property 5: Map insert and lookup consistency
...
-- Property 6: Set insert and member consistency
...
prop_trim_only_whitespace s =
  ...
  in all (not . isWhitespace) (take 1 trimmed ++ drop (length trimmed - 1) trimmed) === True

-- Property 10: splitBy and concat are inverse operations
prop_splitBy_concat_inverse :: Char -> [String] -> Property
prop_splitBy_concat_inverse delim parts =
  not (null parts) && all (notElem delim) parts ==>
  let joined = concat $ map (++ [delim]) (init parts) ++ [last parts]
...
tests = testGroup "Basic Properties QuickCheck Tests"
  ...
  , fastProperty "splitBy preserves length relationship" prop_splitBy_length
  ...
  , fastProperty "Map insert and lookup are consistent" prop_map_insert_lookup
  , fastProperty "Set insert and member are consistent" prop_set_insert_member
  ...
  , fastProperty "splitBy and concat are inverse operations" prop_splitBy_concat_inverse
  ]
