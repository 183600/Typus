...
prop_ownership_type_show_roundtrip ownType =
  ...
  in property $ length shown > 0 .&&. "Owned" `isInfixOf` shown || "Borrowed" `isInfixOf` shown || "MutBorrowed" `isInfixOf` shown
...
prop_ownership_error_contains_info err =
  let shown = show err
  in case err of
    UseAfterMove var -> property $ var `isInfixOf` shown
    DoubleMove var1 var2 -> property $ var1 `isInfixOf` shown .&&. var2 `isInfixOf` shown
    BorrowWhileMoved var -> property $ var `isInfixOf` shown
    MutBorrowWhileBorrowed var -> property $ var `isInfixOf` shown
    BorrowWhileMutBorrowed var -> property $ var `isInfixOf` shown
    MultipleMutBorrows var -> property $ var `isInfixOf` shown
    UseWhileMutBorrowed var -> property $ var `isInfixOf` shown
    OutOfScope var -> property $ var `isInfixOf` shown
    BorrowError msg -> property $ not (null msg) ==> msg `isInfixOf` shown
    ParseError msg -> property $ not (null msg) ==> msg `isInfixOf` shown
    CrossFunctionMove var1 var2 -> property $ var1 `isInfixOf` shown .&&. var2 `isInfixOf` shown
    ParameterMoveMismatch var -> property $ var `isInfixOf` shown
    ControlFlowError msg -> property $ not (null msg) ==> msg `isInfixOf` shown
    PathSensitiveError msg -> property $ not (null msg) ==> msg `isInfixOf` shown
    LoopOwnershipError msg -> property $ not (null msg) ==> msg `isInfixOf` shown
...
prop_lex_simple_expressions expr =
  length expr <= 20 && all (`elem` "abcdefghijklmnopqrstuvwxyz0123456789:=&*() \t\n") expr ==>
...
prop_lex_identifiers ident =
  not (null ident) && all isAlphaNum (head ident : tail ident) ==>
  let expr = ident ++ " := 5"
      result = lexAll expr
  in property $ True -- Should not crash
  where
...
prop_parse_multiple_statements varNames =
  length varNames <= 3 && all (all isAlphaNum) varNames ==>
  let statements = map (\v -> v ++ " := " ++ "5") varNames
...
prop_built_in_functions_contains_types =
  ...
  in property $ all (`elem` builtInFunctions) typeNames
...
prop_built_in_functions_contains_packages =
  ...
  in property $ all (`elem` builtInFunctions) packages
...
prop_format_multiple_errors errors =
  ...
  in property $ all (`isInfixOf` formatted) errStrs
