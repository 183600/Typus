...
testSuite :: TestTree
testSuite = testGroup "New Utils QuickCheck Properties"
  [ trimProperties
  , splitProperties
  , commentProperties
...
splitProperties = testGroup "Split Properties"
  [ testProperty "splitBy: length matches count of delimiter + 1" $
      \delim s -> 
        let parts = splitBy delim s
            expectedCount = length (filter (== delim) s) + 1
        in delim /= '\0' ==> length parts === expectedCount
        
  , testProperty "splitByCollapsed: never returns empty strings" $
      \delim s -> all (not . null) (splitByCollapsed delim s)
      
  , testProperty "splitByCollapsed: result is subset of splitBy" $
      \delim s -> all (`elem` splitBy delim s) (splitByCollapsed delim s)
  ...
  , testProperty "splitBy: rejoining with delimiter preserves original" $
      \delim s -> delim /= '\0' ==> concat (splitBy delim s) === s
  ...
  , testProperty "splitByCollapsed: consecutive delimiters are collapsed" $
      \delim s ->
        let parts = splitByCollapsed delim s
            hasConsecutive = any (\(a:b:_) -> a == delim && b == delim) (zip s (tail s) ++ repeat ['\0'])
        in hasConsecutive ==> length parts < length (splitBy delim s)
  ]
...
commentProperties = testGroup "Comment Properties"
  ...
  , testProperty "removeLineComments: preserves lines without //" $
      \line -> not ("//" `isPrefixOf` line) ==> removeLineComments line === line
  ...
  , testProperty "removeComments: preserves text outside /* ... */" $
      \prefix suffix ->
        let input = prefix ++ "/* comment */" ++ suffix
            result = removeComments input
        in prefix `isPrefixOf` result && suffix `isSuffixOf` result
  ]
...
indentationProperties = testGroup "Indentation Properties"
  ...
  , testProperty "normalizeIndentation: preserves relative indentation" $
      \lines ->
        ...
        in not (null resultLines) ==> 
           ...
           length resultLines === length (filter (not . null) lines)
  ...
  , testProperty "normalizeIndentation: removes common leading whitespace" $
      \ws content ->
        ...
        in all isSpace ws ==> not (ws `isPrefixOf` result)
  ]
