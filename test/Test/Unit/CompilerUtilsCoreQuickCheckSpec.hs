...
prop_logger_message_processing logger message =
  ...
  in property $ not (null processed) ==> L.length processed >= L.length message - 100 -- Allow some processing variance
...
prop_logger_message_content logger message =
  ...
  in hasContent ==> property $ L.length (L.filter (not . null) wordsInOriginal) >= 1
...
prop_logger_composition logger msg1 msg2 =
  let combined = msg1 ++ " " ++ msg2
      combinedLength = L.length combined
      separateLength = L.length msg1 + L.length msg2 + 1
...
prop_logger_empty_messages logger =
  let emptyMsg = ""
      whitespaceMsg = "   \t\n  "
  in property $ L.length emptyMsg === 0 .&&. L.length (trim whitespaceMsg) === 0
  where
...
prop_logger_unicode_messages logger baseMsg =
  let unicodeMsg = baseMsg ++ "测试🚀café naïve"
      hasUnicode = L.any (> '\x7F') unicodeMsg
  in hasUnicode ==> property $ L.length unicodeMsg >= L.length baseMsg
...
prop_logger_long_messages logger baseMsg multiplier =
  ...
  let longMsg = L.concat (replicate multiplier baseMsg)
      expectedLength = multiplier * L.length baseMsg
  in property $ L.length longMsg === expectedLength
...
prop_logger_message_invariants logger message =
  let processed = message
      originalLength = L.length message
...
prop_logger_special_characters logger baseMsg =
  ...
  in property $ L.length msgWithSpecial === L.length baseMsg * 2 + L.length specialChars
...
prop_logger_case_handling logger message =
  let upperMsg = map toUpper message
      ...
      originalLength = L.length message
  in property $ L.length upperMsg === originalLength .&&. L.length lowerMsg === originalLength
...
prop_logger_repeated_messages logger message count =
  count >= 0 && count <= 50 ==>
  let repeatedMsg = L.concat (replicate count message)
      expectedLength = count * L.length message
  in property $ L.length repeatedMsg === expectedLength
