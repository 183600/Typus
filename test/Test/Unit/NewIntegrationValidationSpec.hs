...
newIntegrationValidationSpec = testGroup "New Integration Validation Tests"
  [ testCase "Parser and SourceLocation integration" $ do
  ...
  , testCase "Utils and ErrorHandling integration" $ do
      ...
      case parseTypus malformedCode of
        ...
        Right _ -> do
          ...
          let err = errorWithContext "Invalid directive value" pos 
                                      "Expected 'on' or 'off'" 
                                      ["Change 'maybe' to 'on' or 'off'"]
          
          let formatted = formatErrorWithSuggestions err
          assertBool "Error should contain context" $ "Expected 'on' or 'off'" `isInfixOf` formatted
          assertBool "Error should contain suggestions" $ "Change 'maybe' to 'on' or 'off'" `isInfixOf` formatted
...
prop_error_context_with_utils msg ctx = 
  not (null msg) && not (null ctx) ==> 
    ...
    in msg `isInfixOf` formatted &&
       ctx `isInfixOf` formatted &&
       trim ctx `isInfixOf` formatted
  ...
  not (null blockContents) ==> 
    ...
    in case parseTypus code of
         ...
         Right typusFile -> 
           ...
           in length parsedBlocks >= length blockContents
...
prop_directive_and_location_tracking directive = 
  ...
  in case parseTypus code of
       ...
       Right typusFile -> 
         ...
         in not (null blocks) && all (\block -> isValidSpan (cbSpan block)) blocks
...
integrationQuickCheckProperties = testGroup "Integration QuickCheck Properties"
  ...
  , testProperty "directive and location tracking" prop_directive_and_location_tracking
