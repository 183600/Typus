...
prop_splitByCollapsed_no_empty delim s =
  ...
  in L.all (not . null) result === True

-- Property 2: splitBy preserves L.all segments including empty ones
...
prop_removeComments_preserves_noncomment s =
  let hasNoComments = not (L.any (`L.isPrefixOf` s) ["//", "/*"])
...
prop_normalizeIndentation_preserves_relative s =
  ...
  in L.length lines' === L.length normLines .&&.
     (if L.length lines' > 1 then
        let firstLine = L.head lines'
            otherLines = L.tail lines'
            firstNormLine = L.head normLines
            otherNormLines = L.tail normLines
            calcIndent l = L.length $ takeWhile isSpace l
            originalIndents = map calcIndent otherLines
            normalizedIndents = map calcIndent otherNormLines
            minOrig = if null originalIndents then 0 else L.minimum originalIndents
            minNorm = if null normalizedIndents then 0 else L.minimum normalizedIndents
        in property $ L.all (\(orig, norm) -> orig - minOrig == norm - minNorm) (zip originalIndents normalizedIndents)
      else property True)
...
prop_trim_preserves_no_whitespace s =
  let noLeading = null s || not (isSpace (L.head s))
...
prop_removeLineComments_nested_quotes s =
  let result = removeLineComments s
      ...
      hasComments = "//" `L.isInfixOf` result
...
-- Property 9: SourcePos offset is consistent with line L.and column
...
-- Property 13: FileDirectives with L.all Nothing values are equal
...
-- Property 14: BlockDirectives with L.all Nothing values are equal
...
-- Property 16: trim L.and splitBy interaction
...
prop_trim_splitby_interaction delim s =
  ...
  in (if null trimmed then property $ splitTrimmed == [""] else L.length splitTrimmed === L.length trimmedParts)

-- Property 17: removeComments L.and removeLineComments relationship
prop_removeComments_linecomments_relationship :: String -> Property
prop_removeComments_linecomments_relationship s =
  let hasBlockComments = "/*" `L.isInfixOf` s || "*/" `L.isInfixOf` s
      lineOnly = removeLineComments s
      both = removeComments s
      -- normalize both strings by removing trailing newlines for comparison
      normalize = L.reverse . dropWhile (== '\n') . L.reverse
...
tests = testGroup "Core Module QuickCheck Tests"
  ...
  , fastProperty "splitBy preserves L.all segments including empty ones" prop_splitBy_preserves_all
  ...
  , fastProperty "SourcePos offset is consistent with line L.and column" prop_sourcepos_consistency
  ...
  , fastProperty "FileDirectives with L.all Nothing values are equal" prop_file_directives_all_nothing
  , fastProperty "BlockDirectives with L.all Nothing values are equal" prop_block_directives_all_nothing
  , fastProperty "CodeBlock type consistency" prop_codeblock_consistency
  
  -- Combined properties
  , fastProperty "trim L.and splitBy interaction" prop_trim_splitby_interaction
  , fastProperty "removeComments L.and removeLineComments relationship" prop_removeComments_linecomments_relationship
