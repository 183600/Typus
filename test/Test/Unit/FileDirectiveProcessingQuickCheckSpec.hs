...
module Test.Unit.FileDirectiveProcessingQuickCheckSpec (tests) where
...
-- Generate file directive values
genFileDirectiveValue :: Gen Bool
genFileDirectiveValue = arbitrary

-- Generate file directive strings
genFileDirectiveString :: Gen String
genFileDirectiveString = do
  directiveType <- elements ["ownership", "dependent-types", "constraints"]
  value <- genFileDirectiveValue
  return $ "// @" ++ directiveType ++ ": " ++ show value

-- Generate block directive strings
genBlockDirectiveString :: Gen String
genBlockDirectiveString = do
  directiveType <- elements ["ownership", "dependent-types", "constraints"]
  value <- genFileDirectiveValue
  return $ "// @" ++ directiveType ++ ": " ++ show value

-- Generate code with file directives
genCodeWithFileDirectives :: Gen String
genCodeWithFileDirectives = do
...
hasBlockDirective block =
  ...
  in directives /= defaultBlockDirectives
