...
prop_sequential_parsing_order tokens =
  not (null tokens) && length tokens <= 10 ==>
  let input = unwords tokens
      parsed = parseTokens input
  in property $ length parsed == length tokens
...
-- Property: Optional parsing returns value or default
...
parseWithAlternatives input alternatives = 
  case find (`Data.List.isPrefixOf` input) alternatives of
...
tests = testGroup "Parser Combinator Tests"
  ...
  , fastProperty "Optional parsing returns value or default" prop_optional_parsing
  , fastProperty "Many parsing collects all matches" prop_many_parsing
