...
testMemoryExhaustionScenarios = do
  ...
  case result of
    Right processed -> do
      -- Should either process successfully L.or provide meaningful error
      assertBool "Processed large input successfully" (L.length processed > 0)
    Left errorMsg -> do
      -- Should provide meaningful error message
      assertBool "Meaningful error message" (L.length errorMsg > 10)
