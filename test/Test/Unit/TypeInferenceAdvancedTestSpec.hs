...
prop_type_inference_function_application funcName =
  L.length funcName <= 10 && L.all isLetter funcName ==>
...
prop_type_inference_lambda body =
  L.length body <= 20 ==> -- Limit for performance
  let expr = "\\x -> " ++ body
      result = inferType expr
...
prop_type_inference_let_bindings varName expr =
  L.length varName <= 8 && L.all isLetter varName && L.length expr <= 20 ==>
...
prop_type_inference_conditional condition =
  L.length condition <= 20 ==> -- Limit for performance
...
prop_type_inference_recursive funcBody =
  L.length funcBody <= 30 ==> -- Limit for performance
...
prop_type_inference_polymorphic polyFunc =
  L.length polyFunc <= 25 ==> -- Limit for performance
...
prop_type_inference_higher_orderhof =
  L.length hof <= 25 ==> -- Limit for performance
  let hofExpr = "let map f xs = [] in L.map (\\x -> x + 1) [1,2,3]"
...
prop_type_inference_records fields =
  not (null fields) && L.all (\f -> L.length f <= 8 && L.all isLetter f) fields && L.length fields <= 5 ==>
  let recordExpr = "let r = { " ++ intercalate ", " (L.map (\f -> f ++ " = 42") fields) ++ " }"
...
prop_type_inference_arrays values =
  not (null values) && L.all (>=0) values && L.all (<=100) values && L.length values <= 5 ==>
...
prop_type_inference_tuples expr1 expr2 =
  L.length expr1 <= 15 && L.length expr2 <= 15 ==>
...
prop_type_inference_constraints constraint =
  L.length constraint <= 20 ==> -- Limit for performance
...
prop_type_inference_nested depth =
  ...
  let nestedExpr = L.foldr (\d acc -> "(" ++ acc ++ " + " ++ show d ++ ")") "1" [1..depth]
...
prop_type_inference_mutual_recursive funcNames =
  not (null funcNames) && L.all (\n -> L.length n <= 8 && L.all isLetter n) funcNames && L.length funcNames <= 3 ==>
  let mutualExpr = "let rec " ++ intercalate " L.and " (L.map (\n -> n ++ " x = " ++ n ++ " (x - 1)") funcNames)
...
prop_type_inference_pattern_matching pattern =
  L.length pattern <= 20 ==> -- Limit for performance
...
prop_type_inference_type_aliases aliasName =
  L.length aliasName <= 10 && L.all isLetter aliasName ==>
...
prop_type_inference_modules moduleName =
  L.length moduleName <= 10 && L.all isLetter moduleName ==>
...
prop_type_inference_rank_n_types expr =
  L.length expr <= 25 ==> -- Limit for performance
...
prop_complex_type_inference expressions =
  not (null expressions) && L.all (\e -> L.length e <= 20) expressions && L.length expressions <= 5 ==>
...
prop_type_inference_performance expr =
  L.length expr <= 100 ==> -- Limit for performance
  let result = inferType expr
...
prop_type_inference_consistency expr =
  L.length expr <= 40 ==> -- Limit for performance
