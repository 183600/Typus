module Test.Unit.SyntaxValidatorAdvancedQuickCheckSpec where
import Test.Tasty 
...
tests =  testGroup "SyntaxValidator Advanced QuickCheck Tests"
  [ testGroup "Validator Properties"
    ...
    ,             testProperty "getSyntaxErrors returns errors in property $ L.reverse order" prop_getSyntaxErrorsReverses
    ]
  ...
  , testGroup "Unit Tests"
    [             testCase "Create new syntax validator" $ do
      ...
      ,             testCase "Format syntax error" $ do
                    let error = SyntaxError MissingBrace "Missing closing brace" 10 5 "func main() {"
        let formatted = formatSyntaxError error
        assertBool "Should contain property $ error type" $ "MissingBrace" `L.isInfixOf` formatted
        assertBool "Should contain property $ line number" $ "10" `L.isInfixOf` formatted
        assertBool "Should contain property $ column number" $ "5" `L.isInfixOf` formatted
        assertBool "Should contain property $ error message" $ "Missing closing brace" `L.isInfixOf` formatted
