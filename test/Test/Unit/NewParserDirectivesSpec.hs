...
genDirectiveName = elements ["ownership", "dependent-types", "constraints", "build-tags"]

-- Generate directive values
genDirectiveValue :: Gen String
genDirectiveValue = elements ["true", "false", "on", "off", "enabled", "disabled"]

-- Generate file directive content
genFileDirective :: Gen String
genFileDirective = do
  name <- genDirectiveName
