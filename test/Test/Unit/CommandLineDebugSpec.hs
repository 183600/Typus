module Test.Unit.CommandLineDebugSpec where
import Test.Tasty 
...
breakpoints1 <- readIORef (cldBreakpoints config)
              assertEqual "Should have 2 breakpoints" (Set.fromList ["test-location-1", "test-location-2"]) breakpoints1
...
watchVars1 <- readIORef (cldWatchVariables config)
              assertEqual "Should have 2 watch variables" (Map.fromList [("var1", "value1"), ("var2", "42")]) watchVars1
  ...
  watchVars2 <- readIORef (cldWatchVariables config)
              assertEqual "Should have 1 variable after removal" (Map.singleton "var2" "42") watchVars2
...
testCallStackManagement =             testCase "Call stack management" $ do
  ...
  callStack1 <- getCallStack config
              assertEqual "Should have 3 functions in call stack" ["function3", "function2", "function1"] callStack1
  
  -- Pop from call stack
  popCallStack config
  callStack2 <- getCallStack config
              assertEqual "Should have 2 functions after pop" ["function2", "function1"] callStack2
