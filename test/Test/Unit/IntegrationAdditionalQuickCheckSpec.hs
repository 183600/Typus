...
module Test.Unit.IntegrationAdditionalQuickCheckSpec (tests) where
...
-- Generate complex Typus file content with multiple features
genComplexTypusFile :: Gen String
genComplexTypusFile = do
  hasFileDirectives <- arbitrary
  hasBuildTags <- arbitrary
  numBlocks <- choose (1, 3)
  
  let fileDirectives = if hasFileDirectives
        then ["//! ownership: on", "//! dependent_types: true"]
        else []
  
  let buildTags = if hasBuildTags
        then ["//go:build linux", "// +build amd64"]
        else []
  
  blocks <- vectorOf numBlocks $ genBlockWithDirectives
