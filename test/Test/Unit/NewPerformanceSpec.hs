module Test.Unit.NewPerformanceSpec where
...
import Test.Tasty.QuickCheck (testProperty, Property, (==>), Positive)
      ...
      case parseTypus largeCode of
        ...
        Right typusFile -> do
          ...
          assertBool "Should parse large function" $ not (L.null (tfBlocks typusFile)
    ...
    ,             testCase "Utils performance with large strings" $ do
      ...
      assertBool "Split should produce correct number of parts" $ L.length                               splitResult == 15001
    ...
    ,             testCase "SourceLocation performance with many positions" $ do
      ...
      assertBool "Should create expected number of positions" $ L.length                               positions == 10000
      assertBool "Should create valid spans" $ L.length                               validSpans == 5000
    ...
    ,             testCase "Error handling performance with many errors" $ do
      ...
      assertBool "Should collect L.all errors" $ getErrorCount                               collection == 1000
      assertBool "Should find blocking errors" $ L.length                               blocking == 1000
      assertBool "Should format L.all errors" $ not (null formatted)
    ...
    ,             testCase "Memory efficiency with repeated operations" $ do
      ...
      let successful = L.length [() | Right _ <- results]
      ...
      let forcedStrings = force stringResults `seq` L.length stringResults
      end2 <- getCPUTime
      let diff2 = fromIntegral (end2 - start2) / (10^12)
      
      assertBool "Repeated string operations should be fast" $ diff2 < 0.5
      assertBool "Should produce expected results" $ L.all (== "hello, world, test, string") stringResults
  ]
...
prop_parse_performance_scales_linearly (Positive n) = 
  n <=                               100 ==>  -- Limit size for practical testing
    ...
    in case parseTypus code of
         ...
         Right typusFile -> L.length (tfBlocks typusFile) <= 1  -- Should not create excessive blocks

prop_string_operations_performance :: String -> Property
prop_string_operations_performance                               s = 
  L.length s <=                               1000 ==>  -- Limit size for practical testing
    let trimmed = trim s
                                      split = splitBy ',' s
                                      removedComments = removeLineComments s
    in L.length trimmed <= L.length s &&
       L.length split >= 1 &&
       L.length removedComments <= L.length s
...
prop_position_creation_performance (Positive n) = 
  n <=                               1000 ==> 
    ...
    in L.length                               positions == n &&
       L.length                               spans == n `div` 2 &&
       L.all isValidSpan spans
...
prop_error_collection_performance (Positive n) = 
  n <=                               1000 ==> 
    ...
    in getErrorCount                               collection == n &&
       L.length (getAllErrors collection) == n

prop_deepseq_performance :: [String] -> Property
prop_deepseq_performance                               strings = 
  L.length strings <=                               100 ==> 
let processed = L.map (trim . removeLineComments) strings
                                      forced =  force processed
    in L.length                               forced == L.length strings
