...
import Dependencies.AST (AST(..), Statement(..), TypeExpr(..), Constraint)
    ...
    , testGroup "Cross-module Test.Unit.DependenciesCycleDetectionSpec detection"
        [             testCase "detects cycles across type definitions" $ do
                  ...
                  , "type D<T> = A"
                                              ...
                                              errors = analyzeDependentTypes source
            assertBool "Should detect cross-module Test.Unit.DependenciesCycleDetectionSpec $ L.any isCycleError errors
