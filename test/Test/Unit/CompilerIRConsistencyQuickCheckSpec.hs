...
-- ============================================================================
-- Arbitrary instances for Compiler IR types
-- ============================================================================
...
instance Arbitrary IRType where
    arbitrary = oneof
        ...
        , return IRVoid
        , IRPtr <$> arbitrary
        , IRArray <$> arbitrary <*> arbitrary
        , IRFunc <$> listOf arbitrary <*> arbitrary
        ]
...
instance Arbitrary IRExpression where
    arbitrary = oneof
        [ IRConst <$> arbitrary
        , IRVar <$> identifier
        , IRBinaryOp <$> arbitraryOp <*> arbitrary <*> arbitrary
        , IRUnaryOp <$> arbitraryUnaryOp <*> arbitrary
        , IRCall <$> identifier <*> listOf arbitrary
        , IRIndex <$> arbitrary <*> arbitrary
        , IRMemberAccess <$> arbitrary <*> identifier
        ]
      where
        identifier = elements ["x", "y", "value", "result", "data", "ptr"]
        arbitraryOp = elements ["+", "-", "*", "/", "==", "!=", "<", ">", "<=", ">="]
        arbitraryUnaryOp = elements ["!", "-"]

instance Arbitrary IRStatement where
    arbitrary = oneof
        [ IRDecl <$> identifier <*> arbitrary <*> arbitrary
        , IRAssign <$> identifier <*> arbitrary
        , IRReturn <$> oneof [return Nothing, Just <$> arbitrary]
        , IRIf <$> arbitrary <*> listOf arbitrary <*> listOf arbitrary
        , IRWhile <$> arbitrary <*> listOf arbitrary
        , IRCallStmt <$> identifier <*> listOf arbitrary
        , IRBreak
        , IRContinue
        ]
      where
...
tests = testGroup "Compiler IR Consistency QuickCheck Tests"
    ...
    , testGroup "IRModule Properties"
        [ testProperty "IRModule preserves module name" $
