module Test.Unit.NewParserErrorRecoveryQuickCheckSpec where
...
prop_errorPositionsWithinBounds                               input =
  ...
  in property $ counterexample ("input L.length=" ++ show inputLen ++ ",                               result =" ++ show result) $
     case result of
       Left err -> 
         -- Error message should reference positions within property $ bounds
