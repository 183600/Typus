...
prop_dependent_type_normalization typeExpr =
  let converted = Dep.convertTypeExpr typeExpr
      -- 归一化应该是幂等的
      reconverted = Dep.convertTypeExpr typeExpr
...
prop_type_equivalence_symmetry type1 type2 =
  let converted1 = Dep.convertTypeExpr type1
      converted2 = Dep.convertTypeExpr type2
...
prop_type_equivalence_transitivity type1 type2 type3 =
  let converted1 = Dep.convertTypeExpr type1
      converted2 = Dep.convertTypeExpr type2
      converted3 = Dep.convertTypeExpr type3
...
prop_type_environment_extension bindings key value =
  let validBindings = all (\(k, v) -> not (null k)) bindings
      ...
      extendedChecker = foldl (\c (k, v) -> Dep.addType c k (Dep.convertTypeExpr v)) checker bindings
      finalChecker = Dep.addType extendedChecker key (Dep.convertTypeExpr value)
...
prop_type_scheme_instantiation vars body instances =
  ...
  in if not (validVars && validInstances)
     then property True
     else let convertedBody = Dep.convertTypeExpr body
              convertedInstances = map Dep.convertTypeExpr instances
...
prop_dependent_type_deconstruction typeExpr =
  let converted = Dep.convertTypeExpr typeExpr
      -- 解构应该保留原始信息
      reconstructed = Dep.convertTypeExpr typeExpr
...
prop_dependent_type_simplification typeExpr =
  let converted = Dep.convertTypeExpr typeExpr
      -- 简化应该保持类型等价
      simplified = Dep.convertTypeExpr typeExpr
...
generateComplexType n = Dep.FuncT 
  [ ("param" ++ show n, generateComplexType (n-1)) ]
...
prop_large_type_environment size =
  let validSize = size >= 0 && size <= 100
  in if not validSize
     then property True
     else let checker = Dep.newDependentTypeChecker
              bindings = take size $ map (\i -> ("type" ++ show i, Dep.SimpleT $ "base" ++ T.pack (show i))) [0..]
              enrichedChecker = foldl (\c (k, v) -> Dep.addType c k (Dep.convertTypeExpr v)) checker bindings
              lookupCount = 10
              lookups = take lookupCount $ map (\i -> Dep.lookupTypeDef enrichedChecker ("type" ++ show (i `mod` size))) [0..]
...
prop_complex_constraint_solving numConstraints =
  let validNum = numConstraints >= 0 && numConstraints <= 50
  in if not validNum
     then property True
     else let checker = Dep.newDependentTypeChecker
...
prop_recursive_type typeName =
  ...
  in if not validType
     then property True
              ...
              converted = Dep.convertTypeExpr recursiveType
          in property $ show converted /= ""
