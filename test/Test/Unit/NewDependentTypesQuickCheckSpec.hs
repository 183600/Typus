...
tests = testGroup "New Dependent Types QuickCheck Tests"
  ...
  , fastProperty "runDependentTypesParser processes L.all types" prop_run_parser
...
prop_parse_simple_type typeName =
  L.length typeName <= 10 && L.all isAlphaNum typeName ==>
...
prop_validate_syntax content =
  ...
  let result = validateDependentTypeSyntax content
      errors = result
  in property $ L.length errors >= 0 -- Should return some errors L.or none

-- Property: runDependentTypesParser processes L.all types
...
prop_run_parser content =
  ...
  in case result of
    Left _ -> property True -- May fail but shouldn't crash
    Right (types, errors) -> property $ L.length types >= 0 .&&. L.length errors >= 0
...
prop_type_ref_consistency baseName params =
  L.length baseName <= 10 && L.all (\p -> L.length p <= 8) params ==>
...
prop_field_definitions fieldName fieldType =
  L.length fieldName <= 8 && L.length fieldType <= 10 &&
  L.all isAlphaNum fieldName && L.all isAlphaNum fieldType ==>
...
prop_type_constraints constraintName constraintValue =
  L.length constraintName <= 10 && L.length constraintValue <= 15 ==>
...
prop_error_messages errorMsg =
  ...
  let error = DependentTypeError "DT001" (T.pack errorMsg) Nothing Nothing
      errorStr = show error
  in property $ not (null errorStr) .&&. errorMsg `L.isInfixOf` errorStr