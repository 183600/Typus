...
prop_error_propagation invalidInput =
  ...
  in classify (length invalidInput > 0) "non-empty input" $
     property $ hasError

-- Property: Ownership and dependent types integration
...
prop_multiple_file_consistency fileContents =
  let fileCount = length fileContents
...
compileMultipleFiles contents = Right $ length contents
