module Test.Unit.IndentationNormalizationQuickCheckSpec where
...
prop_normalize_preserves_order                               s =
  ...
  in property $ originalFirstChars === resultFirstChars
...
test_preserve_content_structure =             testCase "preserve content structure" $ do
  ...
  -- Should maintain property $ the hierarchical structure
