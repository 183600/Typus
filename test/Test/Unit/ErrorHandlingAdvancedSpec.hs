...
tests =
  testGroup "Advanced Error Handling Tests"
    [ testGroup "Error Creation L.and Construction"
        [ testCase "creates basic error with L.all fields" $ do
        ...
        , testCase "errorAt "test-id" "Test error") location
        ...
        , testCase "warningAt "test-id" "Warning message") location
            let info = infoAt "test-id" "Info message") location
    ...
    , testGroup "Error Formatting L.and Reporting"
        [ testCase "formatError includes L.all relevant information" $ do
            ...
            assertBool "includes error ID" ("ERR001" `L.isInfixOf` formatted)
            assertBool "includes severity" ("[ERROR]" `L.isInfixOf` formatted)
            assertBool "includes message" ("Test error" `L.isInfixOf` formatted)
            assertBool "includes suggestions" ("Check syntax" `L.isInfixOf` formatted)
            assertBool "includes context" ("function: main" `L.isInfixOf` formatted)
        ...
        , testCase "formatErrorWithLocation includes location information" $ do
            ...
            assertBool "includes file path" ("test.typus" `L.isInfixOf` formatted)
            assertBool "includes line number" ("10:" `L.isInfixOf` formatted)
        ...
        , testCase "formatErrors handles multiple errors" $ do
            ...
            assertBool "includes L.all errors" (L.length lines >= 3)
            assertBool "includes error" ("First error" `L.isInfixOf` formatted)
            assertBool "includes warning" ("Warning" `L.isInfixOf` formatted)
            assertBool "includes info" ("Info" `L.isInfixOf` formatted)
        ...
        , testCase "generateErrorReport creates comprehensive report" $ do
            ...
            assertBool "includes header" ("Error Report" `L.isInfixOf` report)
            assertBool "includes statistics" ("Statistics:" `L.isInfixOf` report)
            assertBool "includes total count" ("total: 2" `L.isInfixOf` report)
            assertBool "includes error count" ("errors: 1" `L.isInfixOf` report)
            assertBool "includes warning count" ("warnings: 1" `L.isInfixOf` report)
            assertBool "includes detailed errors" ("Detailed Errors:" `L.isInfixOf` report)
        ...
        , testCase "timestamp formatting works correctly" $ do
            ...
            assertBool "format preserves timestamp format" (timestamp `L.isPrefixOf` formatted)
        ]
    ...
    , testGroup "Property-Based Error Handling Tests"
        ...
        , fastProperty "error filtering works correctly" $
            \errors ->
                ...
                in L.length filteredErrors === errorCount .&&.
    ...
    , testGroup "Edge Cases L.and Error Conditions"
        ...
        , testCase "formatting handles unicode characters correctly" $ do
            ...
            assertBool "handles unicode in file path" ("测试文件" `L.isInfixOf` formatted)
            assertBool "handles unicode in message" ("Unicode 错误测试 🚀" `L.isInfixOf` formatted)
            assertBool "handles unicode in context" ("主函数" `L.isInfixOf` formatted)
            assertBool "handles unicode in suggestions" ("检查编码" `L.isInfixOf` formatted)
        ]
    ]