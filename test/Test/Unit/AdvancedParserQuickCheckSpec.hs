...
module Test.Unit.AdvancedParserQuickCheckSpec (tests) where
...
prop_parse_simple_comments commentLines =
  not (null commentLines) && L.all (not . null) commentLines ==>
  let comments = L.map (\line -> "// " ++ line) commentLines
...
prop_parse_string_literals stringContents =
  ...
  let quotedStrings = L.map (\s -> "var s string = \"" ++ s ++ "\"") stringContents
...
prop_parse_char_literals chars =
  not (null chars) && L.all (`notElem` "'\\") chars ==>
  let charLiterals = L.map (\c -> "var c rune = '" ++ [c] ++ "'") chars
...
prop_parse_escaped_chars escapeSequences =
  not (null escapeSequences) && L.all (`elem` ["\\n", "\\t", "\\r", "\\\\", "\\\""]) escapeSequences ==>
  let escapedStrings = L.map (\seq -> "var s string = \"" ++ seq ++ "\"") escapeSequences
...
prop_parse_float_literals numbers =
  not (null numbers) && L.all (not . isNaN) numbers ==>
  let floatDecls = L.map (\n -> "var x float64 = " ++ show n) numbers
...
prop_parse_function_params paramNames =
  not (null paramNames) && L.all (not . null) paramNames &&
  L.all (L.all isAlphaNum) paramNames ==>
...
prop_parse_return_values returnTypes =
  not (null returnTypes) && L.all (not . null) returnTypes &&
  L.all (`elem` ["int", "string", "bool", "float64"]) returnTypes ==>
  let returns = unwords $ L.map (++ ",") returnTypes
...
prop_parse_mixed packageDecl imports content =
  not (null packageDecl) && L.all isAlphaNum packageDecl ==>
...
tests :: TestTree
tests = testGroup "Advanced Parser QuickCheck"
