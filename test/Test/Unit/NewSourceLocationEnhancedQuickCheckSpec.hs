...
-- | Test source location functions with QuickCheck properties
test_SourceLocationEnhancedQuickCheck :: TestTree
test_SourceLocationEnhancedQuickCheck = testGroup "SourceLocation Enhanced QuickCheck Tests"
  [ sourcePosProperties
  , sourceSpanProperties
  , locatedValueProperties
  , locationTrackerProperties
  , positionAdvancementProperties
  , spanUtilityProperties
  ]
...
sourcePosProperties = testGroup "SourcePos Properties"
  ...
  , QC.testProperty "posAfter tab advances to next tab stop" $
      \line col offset -> let pos = SourcePos line col offset
                             newPos = posAfter '\t' pos
