...
-- | Multi-module Test.Unit.NewIntegrationQuickCheckPropertySpec tests
multiModuleIntegration :: TestTree
multiModuleIntegration = testGroup "Multi-module Test.Unit.NewIntegrationQuickCheckPropertySpec
  ...
  ,             testProperty "module Test.Unit.NewIntegrationQuickCheckPropertySpec preserves data integrity" $
    \input -> 
      let parseResult = parseTypus input
      in case parseResult of
        Left _ -> property True
  ...
  ,             testProperty "cross-module Test.Unit.NewIntegrationQuickCheckPropertySpec recovery" $
    \input -> 
      let parseResult = parseTypus input
      in case parseResult of
        Left parseErr -> 
...
edgeCaseIntegrationProperties = testGroup "Edge case integration properties"
  ...
  ,             testProperty "concurrent module Test.Unit.NewIntegrationQuickCheckPropertySpec $
    \input1 input2 -> 
