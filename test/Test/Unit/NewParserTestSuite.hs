...
import Parser (parseTypus, TypusAST(..), parseDirective, parseBlock, parseStatement, parseExpression)
...
test_mixed_parameters_type_parsing = do
  ...
  case result of
    ...
    Left err -> assertFailure $ "Failed to parse mixed parameters type: " ++ err

-- | 测试表达式解析
test_expression_parsing :: Assertion
test_expression_parsing = do
  let validExpr = "a + b * c"
      result = parseExpression validExpr
  case result of
    Right expr -> assertEqual "Expression parsed correctly" "BinaryOp" (show expr)
    Left err -> assertFailure $ "Failed to parse expression: " ++ err

-- | 测试复杂表达式解析
test_complex_expression_parsing :: Assertion
test_complex_expression_parsing = do
  let validExpr = "(a + b) * (c - d) / e"
      result = parseExpression validExpr
  case result of
    Right expr -> assertEqual "Complex expression parsed correctly" "BinaryOp" (show expr)
    Left err -> assertFailure $ "Failed to parse complex expression: " ++ err

-- | 测试函数调用解析
test_function_call_parsing :: Assertion
test_function_call_parsing = do
  let validCall = "funcName(arg1, arg2, arg3)"
      result = parseExpression validCall
  case result of
    Right expr -> assertEqual "Function call parsed correctly" "FunctionCall" (show expr)
    Left err -> assertFailure $ "Failed to parse function call: " ++ err

-- | 测试方法调用解析
test_method_call_parsing :: Assertion
test_method_call_parsing = do
  let validCall = "obj.methodName(arg1, arg2)"
      result = parseExpression validCall
  case result of
    Right expr -> assertEqual "Method call parsed correctly" "MethodCall" (show expr)
    Left err -> assertFailure $ "Failed to parse method call: " ++ err

-- | 测试数组访问解析
test_array_access_parsing :: Assertion
test_array_access_parsing = do
  let validAccess = "array[index]"
      result = parseExpression validAccess
  case result of
    Right expr -> assertEqual "Array access parsed correctly" "ArrayAccess" (show expr)
    Left err -> assertFailure $ "Failed to parse array access: " ++ err
