...
prop_split_by_char_preserves_chars delim s =
  ...
  in property (length rejoined >= length s - length (filter (== delim) s))
...
prop_remove_line_comments_preserves_non_commented s =
  ...
  in property (if hasNoComments then processed == s else True)
...
prop_remove_comments_preserves_strings s =
  ...
  in property (if hasStringLiteral 
              then True  -- 简化测试，实际应该检查字符串内容
              else True)
...
prop_trim_preserves_content s =
  ...
  in property (if hasNonWhitespace 
