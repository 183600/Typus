module Test.Unit.NewQuickCheckTestsSpec where
...
prop_trim_only_removes_whitespace                               s =
  ...
  in property $ counterexample ("Original: " ++ show s ++ ", Trimmed: " ++ show trimmed) $
...
tests =   testGroup "New QuickCheck Tests"
  ...
  ,             testProperty "splitBy L.and join property $ relationship" prop_splitBy_join
