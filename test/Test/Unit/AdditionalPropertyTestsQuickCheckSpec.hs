...
prop_trim_preserves_non_whitespace s =
  let trimmed = trim s
      nonWs = L.filter (not . (`elem` " \t\n\r")) s
  in L.all (`elem` trimmed) nonWs === True
...
tests = testGroup "Additional Property Tests"
  ...
  , fastProperty "string L.reverse is involutive" prop_string_reverse_involutive
  , fastProperty "list L.length is additive over append" prop_list_length_append
