module Test.Unit.NewCabalCompilerQuickCheckTestSpec where
...
genCompilerError :: Gen CompilerError
  ...
  suggestions <- listOf (T.pack <$> arbitrary `suchThat` (not . null (SourceSpan (SourcePos 1 1 0 [] (SourcePos 1 1 0))
...
forAll (listOf $ arbitrary `suchThat` (\(x, y (SourceSpan (SourcePos 1 1 0 [] (SourcePos 1 1 0) -> not (null x && null y)) $ \pairs ->
...
prop_round_trip_compilation :: Property
  ...
  forAll genSimpleTypusFile $ \typusFile ->
    ...
    in property $ not (null goCode (SourceSpan (SourcePos 1 1 0 [] (SourcePos 1 1 0)
