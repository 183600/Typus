...
tests =
  testGroup "Dependencies Cycle Detection Properties"
    ...
    , testGroup "Cycle analysis properties"
        ...
        , fastProperty "cycle length is correctly calculated" prop_cycle_length_calculation
        , fastProperty "cycle nodes are all connected" prop_cycle_nodes_connected
        , fastProperty "cycle detection finds all cycles" prop_finds_all_cycles
        ]
...
addNode node graph =
  let newNodes = Map.insert (nodeId node) node (nodes graph)
      newEdges = Set.foldl (\edges dep -> (nodeId node, dep) : edges) (edges graph) (nodeDependencies node)
  in graph { nodes = newNodes, edges = newEdges }

hasCycle :: DependencyGraph -> Bool
hasCycle graph = 
  let nodeIds = Map.keys (nodes graph)
      maxId = length nodeIds
      vertexMap = Map.fromList $ zip nodeIds [0..]
      edgeList = map (\(from, to) -> (Map.findWithDefault 0 from vertexMap, Map.findWithDefault 0 to vertexMap)) (edges graph)
      graphStruct = buildG (0, maxId - 1) edgeList
      sorted = topSort graphStruct
  in length sorted < length nodeIds
...
topologicalSort graph = 
  ...
  then Left "Graph has cycles"
    ...
    let nodeIds = Map.keys (nodes graph)
        ...
        reverseVertexMap = Map.fromList $ map (\(k, v) -> (v, k)) (Map.toList vertexMap)
    in Right $ map (`Map.findWithDefault` "unknown") reverseVertexMap sortedIndices
...
prop_acyclic_no_cycles nodeIds =
  ...
  let nodesList = map (\nodeId -> createNode nodeId "type" Set.empty) nodeIds
      graph = foldl (flip addNode) (DependencyGraph Map.empty []) nodesList
...
prop_cycle_detection_preserves_structure nodeIds =
  not (null nodeIds) && length nodeIds <= 4 && all (not . null) nodeIds && all distinct nodeIds ==>
  let nodesList = map (\nodeId -> createNode nodeId "type" Set.empty) nodeIds
      originalGraph = foldl (flip addNode) (DependencyGraph Map.empty []) nodesList
      _ = hasCycle originalGraph  -- Run cycle detection
  in property $ Map.size (nodes originalGraph) === length nodeIds
  where
    distinct [] = True
    distinct (x:xs) = x `notElem` xs && distinct xs
...
prop_minimal_cycle_detection nodeId1 nodeId2 =
  ...
  length nodeId1 <= 10 && length nodeId2 <= 10 ==>
...
prop_topo_sort_succeeds_acyclic nodeIds =
  ...
  let nodesList = map (\nodeId -> createNode nodeId "type" Set.empty) nodeIds
      graph = foldl (flip addNode) (DependencyGraph Map.empty []) nodesList
  in case topologicalSort graph of
    Right sorted -> property $ length sorted === length nodeIds
...
prop_topo_order_respects_deps dep1 dep2 dep3 =
  ...
  all distinct [dep1, dep2, dep3] &&
  all ((<= 10) . length) [dep1, dep2, dep3] ==>
...
prop_dependency_resolution_terminates nodeIds =
  not (null nodeIds) && length nodeIds <= 4 && all (not . null) nodeIds ==>
  let nodesList = map (\nodeId -> createNode nodeId "type" Set.empty) nodeIds
      graph = foldl (flip addNode) (DependencyGraph Map.empty []) nodesList
  in case topologicalSort graph of
...
prop_dependency_resolution_conservative nodeIds =
  ...
  let nodesList = map (\nodeId -> createNode nodeId "type" Set.empty) nodeIds
      graph = foldl (flip addNode) (DependencyGraph Map.empty []) nodesList
  in case topologicalSort graph of
    Right sorted -> property $ all (`elem` nodeIds) sorted
    Left _ -> property $ True
...
prop_dependency_resolution_preserves_deps fromId toId =
  ...
  length fromId <= 10 && length toId <= 10 ==>
...
prop_cycle_detection_linear_dag nodeCount =
  ...
  let nodeIds = map (\i -> "node_" ++ show i) [1..nodeCount]
...
prop_cycle_detection_memory_bounded nodeCount =
  ...
  let nodeIds = map (\i -> "node_" ++ show i) [1..nodeCount]
      nodesList = map (\nodeId -> createNode nodeId "type" Set.empty) nodeIds
      graph = foldl (flip addNode) (DependencyGraph Map.empty []) nodesList
      memoryUsage = Map.size (nodes graph) + length (edges graph)
