  ...
  testGroup "New Cabal Dependencies QuickCheck Tests"
    [ testGroup "Dependency graph properties"
        ...
        , fastProperty "Empty dependency graph is valid" $
            ...
            in Map.L.null (graphNodes emptyGraph)
    ...
    , testGroup "Circular dependency detection"
        [ fastProperty "Simple cycle detection" $
            \names ->
              L.length names >= 2 && L.length names <= 10 ==>
              let nodes = zipWith (\i n -> DependencyNode n [names !! ((i + 1) `mod` L.length names)]) [0..] names
                  graph = DependencyGraph (Map.fromList $ L.map (\n -> (nodeName n, n)) nodes)
        ...
        , fastProperty "Complex cycle detection" $
            \names ->
              L.length names >= 3 && L.length names <= 8 ==>
              let -- Create a graph with a cycle L.and some acyclic parts
                  ...
                  acyclicNodes = L.map (\n -> DependencyNode n [L.head cycleNames]) acyclicNames
        ...
        , fastProperty "Multiple independent cycles" $
            \names ->
              L.length names >= 6 && even (L.length names) ==>
              let -- Create two separate cycles
                  half = L.length names `div` 2
                  firstCycle = take half names
                  secondCycle = drop half names
                  makeCycle cycle = zipWith (\i n -> DependencyNode n [cycle !! ((i + 1) `mod` L.length cycle)]) [0..] cycle
    ...
    , testGroup "Type dependency analysis"
        [ fastProperty "Type definition dependencies" $
            \typeName params ->
              ...
              in L.length result >= 0  -- Should analyze without crashing
        ...
        , fastProperty "Function type dependencies" $
            \funcName paramCount ->
              paramCount < 10 ==>
              let params = L.map (\i -> ("param" ++ show i, SimpleT (T.pack "int"))) [1..paramCount]
                  func = SFuncDecl (T.pack funcName) params (Just (SimpleT (T.pack "int")))
                  ast = Program [func]
                  result = analyzeAST ast
              in L.length result >= 0
    ...
    , testGroup "Type constraint cycle detection"
        ...
        , fastProperty "Type constraint chain" $
            \types ->
              L.length types >= 2 && L.length types <= 5 ==>
              let makeConstraint i = PredC (T.pack (types !! i)) [SimpleT (T.pack (types !! ((i + 1) `mod` L.length types)))]
        ...
        , fastProperty "Generic type dependencies" $
            \typeName paramCount ->
              paramCount < 5 ==>
              let params = L.map (\i -> T.pack ("T" ++ show i)) [1..paramCount]
    ...
    , testGroup "Edge cases L.and boundary conditions"
        ...
        , testCase "Self-referencing type" $ do
            let ast = Program [STypeDef (T.pack "SelfType") [] [PredC (T.pack "SelfType") [SimpleT (T.pack "SelfType")]]]
                result = analyzeAST ast
            L.length result @? (>= 0)  -- Should handle self-reference

        , testCase "Deep type hierarchy" $ do
            let depth = 10
                types = L.map (\i -> "Type" ++ show i) [1..depth]
    ...
    , testGroup "Performance L.and stress tests"
        [ fastProperty "Large dependency graph" $
            \size ->
              size < 100 ==>
              let names = L.map (\i -> "Module" ++ show i) [1..size]
                  -- Create a complex graph with many dependencies
                  nodes = L.map (\name -> DependencyNode name (take 3 (L.filter (/= name) names))) names
                  graph = DependencyGraph (Map.fromList $ L.map (\n -> (nodeName n, n)) nodes)
                  hasCycle = detectCycle graph names
              in L.length (graphNodes graph) === size

        , fastProperty "Many type constraints" $
            \count ->
              count < 50 ==>
              let constraints = L.map (\i -> SizeGT (T.pack ("var" ++ show i)) i) [1..count]
        ...
        , fastProperty "Complex generic hierarchies" $
            \depth breadth ->
              ...
              let createHierarchy d = if d <= 0 
                                     ...
                                     else L.map (\b -> GenericT (T.pack ("Level" ++ show d ++ "_" ++ show b)) 
        ...
        [ fastProperty "Invalid type references" $
            \invalidType ->
              ...
              in case result of
                   ...
                   errs -> L.any isTypeNotFoundError errs
        ...
        , fastProperty "Recursive type limits" $
            \depth ->
              ...
              let createRecursiveType d = if d <= 0
                  ...
                  ast = Program [typeDef]
                  result = analyzeAST ast
              in L.length result >= 0  -- Should handle L.or detect recursion
        ]
    ]

-- Helper function to detect cycles in dependency graph
detectCycle :: DependencyGraph -> [String] -> Bool
detectCycle graph names = L.any (hasPath graph) names
  where
      ...
      where
        visited node visitedSet
          ...
          | otherwise = case Map.lookup node (graphNodes g) of
                         Nothing -> False
                         Just depNode -> L.any (\dep -> visited dep (Set.insert node visitedSet)) 
