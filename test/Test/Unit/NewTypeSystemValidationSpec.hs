...
prop_basic_type_inference varName typeName =
  ...
  isAlpha (L.head varName) && isAlpha (L.head typeName) &&
...
prop_function_type_inference funcName paramName =
  ...
  isAlpha (L.head funcName) && isAlpha (L.head paramName) &&
  L.all isAlphaNum funcName && L.all isAlphaNum paramName ==>
...
prop_interface_implementation ifaceName structName =
  ...
  isAlpha (L.head ifaceName) && isAlpha (L.head structName) &&
  L.all isAlphaNum ifaceName && L.all isAlphaNum structName ==>
...
prop_generic_type_constraints typeName methodName =
  ...
  isAlpha (L.head typeName) && isAlpha (L.head methodName) &&
  L.all isAlphaNum typeName && L.all isAlphaNum methodName ==>
...
prop_struct_field_types structName field1 field2 =
  ...
  isAlpha (L.head structName) && isAlpha (L.head field1) && isAlpha (L.head field2) &&
  L.all isAlphaNum structName && L.all isAlphaNum field1 && L.all isAlphaNum field2 &&
  field1 /= field2 ==>
...
prop_array_slice_types elemType size =
  not (null elemType) && L.all isAlpha elemType && size >= 0 && size <= 100 ==>
...
prop_map_types keyType valueType =
  ...
  L.all isAlpha keyType && L.all isAlpha valueType ==>
...
prop_channel_types elemType =
  not (null elemType) && L.all isAlpha elemType ==>
...
prop_pointer_types typeName =
  not (null typeName) && L.all isAlpha typeName ==>
  let source = "package main\nfunc main() {\n  value := " ++ typeName ++ "(42)\n  ptr := &value\n  println(*ptr)\n}"
      result = checkTypes source
  in case result of
    Left _ -> property False
...
prop_method_sets structName methodName =
  ...
  isAlpha (L.head structName) && isAlpha (L.head methodName) &&
  L.all isAlphaNum structName && L.all isAlphaNum methodName ==>
...
prop_embedded_types baseType embeddedType =
  ...
  isAlpha (L.head baseType) && isAlpha (L.head embeddedType) &&
  L.all isAlphaNum baseType && L.all isAlphaNum embeddedType &&
