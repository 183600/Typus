...
tests = testGroup "Directive Interaction Tests"
  ...
  , testCase "Block-level dependent types work independently" testBlockDependentTypes
  , testCase "Mixed directives in nested blocks" testNestedMixedDirectives
  , testCase "Directive parsing edge cases" testDirectiveParsingEdgeCases
  , testCase "Directive inheritance behavior" testDirectiveInheritance
  , testProperty "Directive parsing is idempotent" directiveParsingIdempotent
  , testCase "Multiple block directives combine correctly" testMultipleBlockDirectives
  , testCase "Directive precedence rules" testDirectivePrecedence
  ]

-- | Test that file-level ownership directive overrides block-level settings
testFileOwnershipOverride :: Assertion
testFileOwnershipOverride = do
