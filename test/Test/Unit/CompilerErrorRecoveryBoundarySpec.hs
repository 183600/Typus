...
prop_error_recovery_meaningful_locations malformedCode =
  length malformedCode > 5 ==> 
  let result = compileString malformedCode
  in case result of
    Left errors -> property $ not (null errors) .&&. 
      all (\err -> line err > 0 && column err > 0) errors
...
prop_error_recovery_cascading codeWithErrors =
  length codeWithErrors > 10 ==> 
  let result = compileString codeWithErrors
  in case result of
    Left errors -> property $ length errors <= 20 -- Prevent error explosion
    Right _ -> property True
...
prop_error_recovery_unicode baseContent =
  ...
  in case result of
    Left errors -> property $ all (\err -> line err > 0) errors
    Right _ -> property True
...
prop_error_recovery_long_lines multiplier content =
  ...
  let longContent = concat (replicate multiplier content) ++ "{"
...
prop_error_recovery_position_accuracy content errorPos =
  length content > errorPos && errorPos >= 0 ==> 
  let withError = take errorPos content ++ "{" ++ drop errorPos content
      result = compileString withError
  in case result of
    Left errors -> property $ all (\err -> line err >= 1 && column err >= 1) errors
    Right _ -> property True
...
prop_error_recovery_ordered content =
  ...
  in case result of
    Left errors -> property $ 
      length errors >= 2 && 
      all (\(i, err) -> line err >= 1) (zip [0..] errors)
    Right _ -> property True
...
prop_error_recovery_idempotent malformedCode =
  ...
  in case (result1, result2) of
    (Left err1, Left err2) -> property $ length err1 == length err2
...
prop_error_recovery_multiline_positions lines =
  ...
  in case result of
    Left errors -> property $ 
      all (\err -> line err >= 1 && line err <= length lines + 1) errors
    Right _ -> property True
...
compileString input = 
  ...
  then Left [ErrorLocation 1 (length $ takeWhile (/= '{') input) Nothing Nothing]
