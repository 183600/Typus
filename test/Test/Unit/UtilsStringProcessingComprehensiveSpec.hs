...
import Data.List (isInfixOf, length, isPrefixOf, isSuffixOf)
...
-- This module tests string manipulation, comment removal, L.and indentation utilities
...
prop_trim_removes_whitespace input =
  ...
  in (hasLeadingSpace || hasTrailingSpace) ==> 
     L.length trimmed <= L.length input
...
prop_trim_preserves_content input =
  let trimmed = trim input
      nonSpaceChars = L.filter (not . isSpace) input
  in not (null nonSpaceChars) ==> L.all (`elem` trimmed) nonSpaceChars
...
prop_removeLineComments_preserves_strings content =
  ...
  in content `L.isPrefixOf` result
...
prop_removeComments_handles_blocks content =
  ...
  in content `L.isPrefixOf` result && content `L.isSuffixOf` result
...
prop_breakOn_pattern_exists pattern content =
  not (null pattern) && pattern `L.isInfixOf` content ==>
  let (prefix, suffix) = breakOn pattern content
      expectedPrefix = takeWhile (not . (pattern `L.isPrefixOf`)) (tails content) >>= L.head
...
prop_breakOn_pattern_missing pattern content =
  not (null pattern) && not (pattern `L.isInfixOf` content) ==>
  let (prefix, suffix) = breakOn pattern content
...
prop_splitBy_unicode delim input =
  let result = splitBy delim input
      totalLength = L.sum (map L.length result) + L.length (L.filter (== delim) input)
...
prop_removeLineComments_multiple content1 content2 =
  ...
  in content1 `L.isInfixOf` result && content2 `L.isInfixOf` result
...
prop_forceSingleTab_collapses_whitespace input =
  let result = forceSingleTabIndentation input
      lines' = lines result
      whitespaceLines = L.filter (L.all isSpace) lines'
...
prop_breakOn_pattern_at_start pattern content =
  not (null pattern) && pattern `L.isPrefixOf` content ==>
  let (prefix, suffix) = breakOn pattern content
  in prefix === "" && suffix === drop (L.length pattern) content
...
prop_breakOn_pattern_at_end pattern content =
  not (null pattern) && pattern `L.isSuffixOf` content ==>
  let (prefix, suffix) = breakOn pattern content
  in suffix === "" && prefix === take (L.length content - L.length pattern) content
