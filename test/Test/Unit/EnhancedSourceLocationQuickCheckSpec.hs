...
tests :: TestTree
tests =
  testGroup "Enhanced SourceLocation QuickCheck Tests"
    [ testGroup "Source Position Properties"
        [ fastProperty "posAfter advances correctly for newline" prop_posAfterNewline
        , fastProperty "posAfter advances correctly for tab" prop_posAfterTab
        , fastProperty "posAfter advances correctly for regular character" prop_posAfterRegular
        , fastProperty "posAt creates valid position" prop_posAtValid
    ...
    , testGroup "Source Span Properties"
        ...
        , fastProperty "spanFrom creates empty span at position" prop_spanFromEmpty
        ]
    , testGroup "Located Value Properties"
        [ fastProperty "locatedAt creates located value at position" prop_locatedAtCorrect
        , fastProperty "locatedWithSpan creates located value with span" prop_locatedWithSpanCorrect
        , fastProperty "mapLocated preserves location" prop_mapLocatedPreservesLocation
        , fastProperty "locatedValue extracts original value" prop_locatedValueExtracts
        ]
    , testGroup "Position Advancement Properties"
        [ fastProperty "advancePosBy advances correctly for multiple characters" prop_advancePosByCorrect
        , fastProperty "advancePosByText advances correctly for text" prop_advancePosByTextCorrect
        , fastProperty "advancePosByLine advances line number" prop_advancePosByLineCorrect
        , fastProperty "position advancement is consistent" prop_positionAdvancementConsistent
        ]
    , testGroup "Error Location Conversion Properties"
        [ fastProperty "toErrorLocation creates correct error location" prop_toErrorLocationCorrect
        , fastProperty "toErrorLocationWithSpan creates correct error location with range" prop_toErrorLocationWithSpanCorrect
        , fastProperty "error location conversion preserves position info" prop_errorLocationPreservesInfo
        ]
    , testGroup "Utility Properties"
        [ fastProperty "comparePos orders positions correctly" prop_comparePosCorrect
        , fastProperty "position arithmetic is consistent" prop_positionArithmeticConsistent
        , fastProperty "span operations maintain invariants" prop_spanOperationsMaintainInvariants
        ]
    ]
...
-- Property: posAfter advances correctly for newline
...
-- Property: posAfter advances correctly for tab
