...
-- | Run the Typus CLI using the default compiler context.
runCli :: Args -> IO (Either ToolingError ())
runCli args = do
    ctx <- newCompilerContext defaultLogger
    runCliWithContext ctx args

-- | Run the Typus CLI with a custom compiler context (useful for tests).
runCliWithContext :: CompilerContext -> Args -> IO (Either ToolingError ())
runCliWithContext ctx args = runExceptT (dispatch ctx args)
