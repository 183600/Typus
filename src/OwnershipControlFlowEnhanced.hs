...
module OwnershipControlFlowEnhanced
  ( -- 数据模型
    OwnershipType(..)
  , OwnershipError(..)
  , OwnershipAnalyzer
...
data OwnershipError
  ...
  | LoopOwnershipError String       -- 循环所有权错误
  deriving (Show, Eq)
...
data VarState = VarState
  ...
  , vsPathInfo     :: PathInfo       -- 路径敏感信息
  , vsLoopInfo     :: LoopInfo       -- 循环相关信息
  } deriving (Show, Eq)

-- 路径敏感信息
data PathInfo = PathInfo
  { piCreatedInBranch :: Maybe BranchId  -- 在哪个分支中创建
  , piMovedInBranch   :: Maybe BranchId  -- 在哪个分支中被移动
  , piBorrowedInBranch :: Maybe BranchId -- 在哪个分支中被借用
  } deriving (Show, Eq)

-- 循环相关信息
data LoopInfo = LoopInfo
  { liLoopIterations :: Int             -- 循环迭代次数（已知时）
  , liLoopVariables  :: [Name]          -- 循环变量
  , liMovedInLoop    :: Bool            -- 是否在循环中被移动
  , liBorrowedInLoop :: Bool            -- 是否在循环中被借用
  , liLoopId         :: Maybe LoopId    -- 所属循环ID
  } deriving (Show, Eq)
...
data BorrowInfo = BorrowInfo
  { biSource :: !Name
  , biMut    :: !Bool
  , biBranch :: Maybe BranchId  -- 在哪个分支中被借用
  , biLoop   :: Maybe LoopId    -- 在哪个循环中被借用
  } deriving (Show, Eq)
...
data PathState = PathState
  ...
  , psBranchId :: BranchId               -- 当前分支ID
  , psPathId :: PathId                   -- 路径ID
  , psLoopStack :: [LoopId]              -- 循环栈
  , psLoopStates :: Map.Map LoopId LoopState -- 循环状态
  } deriving (Show, Eq)

-- 循环状态
data LoopState = LoopState
  { lsLoopId :: LoopId
  , lsLoopVariables :: [Name]
  , lsIterationCount :: Int
  , lsMovedVariables :: [Name]
  , lsBorrowedVariables :: [Name]
  , lsLoopBodyExecuted :: Bool
  } deriving (Show, Eq)
...
data AnalyzerState = AnalyzerState
  ...
  , asNodeId     :: !Int                   -- 当前节点ID
  , asBranchCounter :: !BranchId           -- 分支计数器
  , asPathCounter :: !PathId               -- 路径计数器
  , asLoopCounter :: !LoopId               -- 循环计数器
  } deriving (Show)
...
buildControlFlowGraph (Program stmts) =
  ...
  where
    ...
    buildCFG stmtList startId = foldl buildStmtCFG ([], [], startId) stmtList
    
    buildStmtCFG :: ([(Int, ControlFlowNode)], [(Int, Int, ControlFlowEdge)], Int) -> Stmt 
                  -> ([(Int, ControlFlowNode)], [(Int, Int, ControlFlowEdge)], Int)
    buildStmtCFG (_nodes, _edges, nextId) stmt = case stmt of
      SIf cond thenBody elseBody _pos ->
      ...
      SFor _mVar collection body _pos ->
        let loopNode = (nextId, CFLoop nextId collection body)
            ...
            newEdges = (nextId, bodyStart, CFLoopEntryEdge) :
                      ...
                      (nextId, joinNodeId, CFLoopExitEdge) :  -- 退出循环的边
                      map (\(from, to, edge) -> (from + nextId, to + nextId, edge)) bodyEdges
        in (newNodes, newEdges, joinNodeId + 1)
      
      SWhile cond body _pos ->
...
collectFunctionSignatures (Program stmts) = concatMap extractFuncSigs stmts
  where
    ...
    extractFuncSigs stmt = case stmt of
      SFunc name params returnType _body _p -> [buildFunctionSig name params returnType]
      _ -> []
...
analyzeWithControlFlow (nodes, edges) = do
  ...
  where
    ...
    analyzeEdge toNodeId edgeType = do
      ...
      case edgeType of
        CFTrueEdge -> do
          ...
          newPath <- createBranchPath branchId
          saveCurrentPath
    ...
    getNextBranchId :: State AnalyzerState BranchId
    getNextBranchId = do
      st <- get
      let nextBranch = asBranchCounter st
      modify $ \s -> s { asBranchCounter = nextBranch + 1 }
      return nextBranch
    
    getNextPathId :: State AnalyzerState PathId
    getNextPathId = do
      st <- get
      let nextPath = asPathCounter st
      modify $ \s -> s { asPathCounter = nextPath + 1 }
      return nextPath
    
    getNextLoopId :: State AnalyzerState LoopId
    getNextLoopId = do
      st <- get
      let nextLoop = asLoopCounter st
      modify $ \s -> s { asLoopCounter = nextLoop + 1 }
      return nextLoop
    
    createBranchPath :: BranchId -> State AnalyzerState PathState
    createBranchPath branchId = do
      st <- get
      let currentPath = asCurrentPath st
      newPathId <- getNextPathId
      return $ currentPath 
        { psBranchId = branchId
        , psPathId = newPathId
        , psVisited = Set.empty  -- 新分支，重置访问记录
        }
    
    createExitPath :: State AnalyzerState PathState
    createExitPath = do
      st <- get
      let currentPath = asCurrentPath st
      newPathId <- getNextPathId
      return $ currentPath 
        { psPathId = newPathId
        , psVisited = Set.empty  -- 新路径，重置访问记录
        }
    
    enterLoop :: LoopId -> State AnalyzerState ()
    enterLoop loopId = do
      st <- get
      let currentPath = asCurrentPath st
          newLoopState = LoopState
            { lsLoopId = loopId
            , lsLoopVariables = []
            , lsIterationCount = 0
            , lsMovedVariables = []
            , lsBorrowedVariables = []
            , lsLoopBodyExecuted = False
            }
          updatedLoopStack = loopId : psLoopStack currentPath
          updatedLoopStates = Map.insert loopId newLoopState (psLoopStates currentPath)
      
      modify $ \s -> s 
        { asCurrentPath = (asCurrentPath s) 
            { psLoopStack = updatedLoopStack
            , psLoopStates = updatedLoopStates
            }
        }
    
    exitLoop :: State AnalyzerState ()
    exitLoop = do
      st <- get
      let currentPath = asCurrentPath st
          updatedLoopStack = case psLoopStack currentPath of
                               [] -> []
                               (_:rest) -> rest
    ...
    setCurrentPath :: PathState -> State AnalyzerState ()
    setCurrentPath path = modify $ \s -> s { asCurrentPath = path }
...
analyzeControlFlowNode node = case node of
  ...
  CFCondition _ cond _thenStmts _elseStmts -> do
    -- 分析条件表达式
    analyzeExpr cond
    -- 路径敏感分析：分别分析两个分支
    -- 分支分析由控制流边处理
  CFLoop _ cond _body -> do
    -- 分析循环条件
    analyzeExpr cond
    -- 分析循环体
    -- 循环体分析由控制流边处理
...
-- 合并路径状态
mergePathStates :: State AnalyzerState ()
mergePathStates = do
  st <- get
  let paths = asPaths st
  case paths of
    [] -> return ()  -- 没有路径
    [_p] -> return ()  -- 只有一个路径，无法合并
    path1:path2:_ -> do
      -- 获取最后两个路径（通常是分支的两个路径）
      -- 合并变量状态
      let mergedVars = mergeVariableStates (psVars path1) (psVars path2)
          mergedBorrows = mergeBorrowStates (psBorrows path1) (psBorrows path2)
          mergedErrors = nub (psErrors path1 ++ psErrors path2)
          mergedLoopStates = mergeLoopStates (psLoopStates path1) (psLoopStates path2)
      
      -- 创建合并后的路径状态
      let mergedPath = PathState
            { psVars = mergedVars
            , psBorrows = mergedBorrows
            , psErrors = mergedErrors
            , psVisited = Set.union (psVisited path1) (psVisited path2)
            , psBranchId = 0  -- 汇合后的分支ID
            , psPathId = psPathId path1  -- 使用第一个路径的ID
            , psLoopStack = psLoopStack path1  -- 使用第一个路径的循环栈
            , psLoopStates = mergedLoopStates
            }
      
      -- 更新状态
      modify $ \s -> s 
        { asCurrentPath = mergedPath
        , asPaths = mergedPath : drop 2 paths  -- 替换前两个路径为合并后的路径
        }

-- 合并变量状态
mergeVariableStates :: Map.Map Name VarState -> Map.Map Name VarState -> Map.Map Name VarState
mergeVariableStates vars1 vars2 = Map.unionWith mergeVarState vars1 vars2
  where
    mergeVarState :: VarState -> VarState -> VarState
    mergeVarState v1 v2 = VarState
      { vsScope = vsScope v1  -- 作用域应该相同
      , vsMoved = vsMoved v1 || vsMoved v2  -- 如果在任一路径被移动，则视为被移动
      , vsBorrowedBy = nub (vsBorrowedBy v1 ++ vsBorrowedBy v2)  -- 合并借用者
      , vsMutBorrower = case (vsMutBorrower v1, vsMutBorrower v2) of
                          (Just b1, Just b2) | b1 == b2 -> Just b1
                          _ -> Nothing  -- 如果有冲突的可变借用，清除
      , vsPathInfo = mergePathInfo (vsPathInfo v1) (vsPathInfo v2)
      , vsLoopInfo = mergeLoopInfo (vsLoopInfo v1) (vsLoopInfo v2)
      }
    
    mergePathInfo :: PathInfo -> PathInfo -> PathInfo
    mergePathInfo p1 p2 = PathInfo
      { piCreatedInBranch = case (piCreatedInBranch p1, piCreatedInBranch p2) of
                              (Just b1, Just b2) | b1 == b2 -> Just b1
                              _ -> Nothing  -- 如果在不同分支创建，清除信息
      , piMovedInBranch = case (piMovedInBranch p1, piMovedInBranch p2) of
                            (Just b1, Just b2) | b1 == b2 -> Just b1
                            _ -> Nothing  -- 如果在不同分支移动，清除信息
      , piBorrowedInBranch = case (piBorrowedInBranch p1, piBorrowedInBranch p2) of
                               (Just b1, Just b2) | b1 == b2 -> Just b1
                               _ -> Nothing  -- 如果在不同分支借用，清除信息
      }
    
    mergeLoopInfo :: LoopInfo -> LoopInfo -> LoopInfo
    mergeLoopInfo l1 l2 = LoopInfo
      { liLoopIterations = max (liLoopIterations l1) (liLoopIterations l2)
      , liLoopVariables = nub (liLoopVariables l1 ++ liLoopVariables l2)
      , liMovedInLoop = liMovedInLoop l1 || liMovedInLoop l2
      , liBorrowedInLoop = liBorrowedInLoop l1 || liBorrowedInLoop l2
      , liLoopId = case (liLoopId l1, liLoopId l2) of
                     (Just id1, Just id2) | id1 == id2 -> Just id1
                     _ -> Nothing
      }

-- 合并借用状态
mergeBorrowStates :: Map.Map Name BorrowInfo -> Map.Map Name BorrowInfo -> Map.Map Name BorrowInfo
mergeBorrowStates borrows1 borrows2 = Map.unionWith mergeBorrowInfo borrows1 borrows2
  where
    mergeBorrowInfo :: BorrowInfo -> BorrowInfo -> BorrowInfo
    mergeBorrowInfo b1 b2 = BorrowInfo
      { biSource = biSource b1  -- 源应该相同
      , biMut = biMut b1  -- 可变性应该相同
      , biBranch = case (biBranch b1, biBranch b2) of
                     (Just br1, Just br2) | br1 == br2 -> Just br1
                     _ -> Nothing  -- 如果在不同分支借用，清除分支信息
      , biLoop = case (biLoop b1, biLoop b2) of
                   (Just l1, Just l2) | l1 == l2 -> Just l1
                   _ -> Nothing  -- 如果在不同循环借用，清除循环信息
      }

-- 合并循环状态
mergeLoopStates :: Map.Map LoopId LoopState -> Map.Map LoopId LoopState -> Map.Map LoopId LoopState
mergeLoopStates loops1 loops2 = Map.unionWith mergeLoopState loops1 loops2
  where
    mergeLoopState :: LoopState -> LoopState -> LoopState
    mergeLoopState ls1 ls2 = LoopState
      { lsLoopId = lsLoopId ls1
      , lsLoopVariables = nub (lsLoopVariables ls1 ++ lsLoopVariables ls2)
      , lsIterationCount = max (lsIterationCount ls1) (lsIterationCount ls2)
      , lsMovedVariables = nub (lsMovedVariables ls1 ++ lsMovedVariables ls2)
      , lsBorrowedVariables = nub (lsBorrowedVariables ls1 ++ lsBorrowedVariables ls2)
      , lsLoopBodyExecuted = lsLoopBodyExecuted ls1 || lsLoopBodyExecuted ls2
      }
...
analyzeStmt stmt = case stmt of
  ...
  SIf cond _thenBody _elseBody _ -> do
    -- 条件已经在控制流节点中分析过
    analyzeExpr cond
    -- 分支分析由控制流图处理
  
  SFor mVar collection _body _ -> do
    -- 分析循环变量声明
    case mVar of
      Just varName -> do
        declareVar varName
        -- 记录循环变量
        recordLoopVariable varName
      Nothing -> return ()
    analyzeExpr collection
    -- 循环体分析由控制流图处理
  
  SWhile cond _body _ -> do
    analyzeExpr cond
    -- 循环体分析由控制流图处理
  ...
  SFunc name params _returnType body _ -> do
    -- 分析函数体，在函数作用域中声明参数
    pushScope
  ...
  -- 处理指令语句
  SDirectiveLine _ _ -> return ()  -- 指令不影响所有权分析
  SDirectiveBlock _ stmts _ -> mapM_ analyzeStmt stmts  -- 分析指令块中的语句

-- 记录循环变量
recordLoopVariable :: Name -> State AnalyzerState ()
recordLoopVariable varName = do
  st <- get
  let currentPath = asCurrentPath st
      currentLoopStack = psLoopStack currentPath
  
  case currentLoopStack of
    [] -> return ()  -- 不在循环中
    (currentLoopId:_) -> do
      let loopStates = psLoopStates currentPath
          currentLoopState = fromMaybe (error "Current loop not found") $ 
                              Map.lookup currentLoopId loopStates
          updatedLoopState = currentLoopState 
            { lsLoopVariables = varName : lsLoopVariables currentLoopState
            }
          updatedLoopStates = Map.insert currentLoopId updatedLoopState loopStates
      
      -- 更新变量的循环信息
      updateVar varName (\vv -> vv 
        { vsLoopInfo = (vsLoopInfo vv) 
            { liLoopVariables = [varName]
            , liLoopId = Just currentLoopId
            }
        })
...
popScope = do
  ...
  let currentPath = asCurrentPath st
      ...
      borrowsToKeep = Map.filterWithKey (\_b info -> 
        Map.member (biSource info) varsToKeep
        ) (psBorrows currentPath)
      loopStatesToKeep = Map.filter (\ls -> not (null (lsLoopVariables ls))) (psLoopStates currentPath)
...
declareVar name = do
  ...
  let lvl = asScope st
      ...
      branchId = psBranchId currentPath
      currentLoopStack = psLoopStack currentPath
      currentLoopId = case currentLoopStack of
                        [] -> Nothing
                        (lid:_) -> Just lid
      ...
      newVar = VarState 
        ...
        , vsMoved = False
        , vsBorrowedBy = []
        , vsMutBorrower = Nothing
        , vsPathInfo = PathInfo (Just branchId) Nothing Nothing
        , vsLoopInfo = LoopInfo 0 [] False False currentLoopId
        }
...
declareParam _funcName (paramName, mType) = do
...
registerBorrowVar borrowName sourceName isMut = do
  ...
  let currentPath = asCurrentPath st
      ...
      borrowInfo = BorrowInfo 
        { biSource = sourceName
        , biMut = isMut
        , biBranch = Just branchId
        , biLoop = currentLoopId
        }
      updatedBorrows = Map.insert borrowName borrowInfo (psBorrows currentPath)
  
  -- 更新源变量的循环信息
  updateVar sourceName (\vv -> vv 
    { vsLoopInfo = (vsLoopInfo vv) 
        { liBorrowedInLoop = True
        }
    })
...
analyzeArgWithParam expr (_paramName, paramType) = case (expr, paramType) of
      ...
      if vsMoved v
        then pushError (DoubleMove name name)
          ...
          then pushError (BorrowWhileMoved name)
            ...
            -- 记录循环中的移动
            recordLoopMove name
            updateVar name (\vv -> vv { vsMoved = True })

recordLoopMove :: Name -> State AnalyzerState ()
recordLoopMove name = do
  st <- get
  let currentPath = asCurrentPath st
      currentLoopStack = psLoopStack currentPath
    ...
    (currentLoopId:_) -> do
      let loopStates = psLoopStates currentPath
          currentLoopState = fromMaybe (error "Current loop not found") $ 
                              Map.lookup currentLoopId loopStates
          updatedLoopState = currentLoopState 
            { lsMovedVariables = name : lsMovedVariables currentLoopState
            }
          updatedLoopStates = Map.insert currentLoopId updatedLoopState loopStates
      
      -- 更新变量的循环信息
      updateVar name (\vv -> vv 
        { vsLoopInfo = (vsLoopInfo vv) 
            { liMovedInLoop = True
            }
        })
...
useVar name = do
  ...
  case Map.lookup name (psBorrows currentPath) of
    Just (BorrowInfo src isMut _ _) -> do
      -- 使用借用变量 - 检查源变量的借用状态
      mv <- lookupVar src
      case mv of
