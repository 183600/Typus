...
module OwnershipControlFlowFixed
  ( -- 数据模型
    OwnershipType(..)
  , OwnershipError(..)
  , OwnershipAnalyzer
...
-- Shared via Ownership.Common.Types.
...
-- 解析条件表达式
parseCondition :: [OwnershipToken] -> (Expr, [OwnershipToken])
parseCondition toks = parseExprUntilRParen toks

-- 解析可选循环变量
parseOptionalLoopVar :: [OwnershipToken] -> (Maybe Name, [OwnershipToken])
parseOptionalLoopVar xs = case skipNL xs of
  (Token (TId name) _ : rest) -> (Just name, rest)
  _ -> (Nothing, xs)

-- 解析可选 else 分支
parseOptionalElse :: [OwnershipToken] -> ([Stmt], [OwnershipToken])
...
-- 解析函数名
parseFuncName :: [OwnershipToken] -> (Name, [OwnershipToken])
parseFuncName toks = case skipNL toks of
  (Token (TId name) _ : rest) -> (name, rest)
  _ -> ("unknown_func", toks)

-- 解析函数参数： (param1 type1, param2 type2, ...)
parseFuncParams :: [OwnershipToken] -> ([(Name, Maybe String)], [OwnershipToken])
parseFuncParams toks = case skipNL toks of
  ...
  _ -> ([], toks)
  where
    collectParams :: [OwnershipToken] -> [(Name, Maybe String)] -> ([(Name, Maybe String)], [OwnershipToken])
...
-- 解析直到右括号
parseExprUntilRParen :: [OwnershipToken] -> (Expr, [OwnershipToken])
parseExprUntilRParen xs = 
  let (exprTokens, rest) = takeUntilRParen xs 0
      expr = tokensToExpr exprTokens
  in (expr, rest)
  where
    takeUntilRParen :: [OwnershipToken] -> Int -> ([OwnershipToken], [OwnershipToken])
    takeUntilRParen [] _ = ([], [])
...
-- 解析直到分号或块结束
parseExprUntilSemicolon :: [OwnershipToken] -> (Expr, [OwnershipToken])
parseExprUntilSemicolon xs =
  let (exprTokens, rest) = takeUntilSemicolon xs
      expr = tokensToExpr exprTokens
  in (expr, rest)
  where
    takeUntilSemicolon :: [OwnershipToken] -> ([OwnershipToken], [OwnershipToken])
    takeUntilSemicolon [] = ([], [])
...
takeExprTokens = go (0 :: Int) (0 :: Int) []
  where
    ...
    go :: Int -> Int -> [OwnershipToken] -> [OwnershipToken] -> ([OwnershipToken], [OwnershipToken])
...
-- 简化的二元表达式解析
parseBinaryExpr :: [OwnershipToken] -> Maybe Expr
...
findBinaryOp :: [OwnershipToken] -> Maybe ([OwnershipToken], BinaryOp, [OwnershipToken], Pos)
...
mergePathStates = do
  ...
  case paths of
    [] -> return ()  -- 没有路径需要合并
    [_] -> return ()  -- 只有一个路径，不需要合并
    path1:path2:_ -> do  -- 获取前两个路径进行合并
...
analyzeStmt stmt = case stmt of
  ...
  SDirectiveBlock _ _ _ -> return ()  -- 指令块不影响所有权分析
...
useVar name = do
  ...
  case Map.lookup name (psBorrows currentPath) of
    Just (BorrowInfo src isMut _) -> do
      -- 使用借用变量 - 检查源变量的借用状态
      mv <- lookupVar src
      case mv of
