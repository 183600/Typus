...
-- Ownership-specific data types are shared via Ownership.Common.Types; this
-- module only specialises the lexical surface for the "advanced" analyzer.
...
data Stmt
  ...
  | SExpr Expr Pos
  | SBlock [Stmt] Pos
  | SFunc Name [(Name, Maybe String)] (Maybe String) [Stmt] Pos  -- func name(params) returnType { body }
...
analyzeStmt st = case st of
  ...
  SFunc name params _ body _ -> do
    ...
    pushScope
    -- Declare function parameters with appropriate ownership
...
-- 声明函数参数
declareParam :: Name -> (Name, Maybe String) -> State AState ()
declareParam _ (paramName, mType) = do
  ...
  case paramType of
    ParamMove -> 
      pushVar paramName (VarState { vsScope = lvl, vsMoved = False, vsBorrowedBy = [], vsMutBorrower = Nothing })
    ParamBorrow ->
      -- For borrow parameters, we need to create a borrow relationship
      -- This is simplified - in reality we'd need to know what we're borrowing from
      pushVar paramName (VarState { vsScope = lvl, vsMoved = False, vsBorrowedBy = [], vsMutBorrower = Nothing })
    ParamMutBorrow ->
      pushVar paramName (VarState { vsScope = lvl, vsMoved = False, vsBorrowedBy = [], vsMutBorrower = Nothing })
